"""Offline validation gate for frozen MPR-XVLA training arms.
...
from tca_map.xvla_task6.data_adapter_smoke import DEFAULT_XVLA_ROOT
...
from tca_map.xvla_task6.training_spec import MODEL_ID, MODEL_REVISION, SPEC_ARTIFACT, TASK6_HDF5_WSL

DEFAULT_OUTPUT = Path("runs/xvla_prior/epoch5_mpr_xvla_offline_validation_step0064.json")
...
def _load_policy(config: OfflineValidationConfig, adapter_dir: Path | None) -> tuple[Any, Any, dict[str, Any]]:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA unavailable for MPR-XVLA offline validation")
...
def run_offline_validation(config: OfflineValidationConfig) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "schema_version": "2026-07-17.epoch5_MPR_XVLA_offline_validation.v1",
        "method": "MPR-XVLA",
        "status": "RUNNING",
        "success": False,
        "decision": "MPR_XVLA_OFFLINE_VALIDATION_RUNNING",
    ...
    try:
        ...
        policies = [
            ...
            ("mpr_xvla_primary", primary_adapter, float(primary_arm["phase_weight_lambda"])),
        ...
        for label in ("mpr_xvla_primary", "uniform_xvla_ablation"):
        ...
        primary = summaries["mpr_xvla_primary"]
        ...
        decision = (
            "MPR_XVLA_OFFLINE_PASS_BEATS_ABLATION"
            if primary_pass and primary_beats_ablation
            else "MPR_XVLA_OFFLINE_SELECTION_NOT_PASSED"
        )
        result.update(
            {
                "status": "COMPLETE",
        ...
        result.update(
            {
                "status": "FAILED",
                "success": False,
                "decision": "MPR_XVLA_OFFLINE_VALIDATION_FAILED",

"""Bounded spec-locked BR-XVLA LoRA trainer.

This module is intentionally narrow.  It only accepts arms from the frozen
BR-XVLA spec, uses the cached X-VLA-Libero model with official PEFT LoRA
attachment, samples task-1 clips through X-VLA's LIBERO reader contract, and
...
from tca_map.xvla_task1.training_spec import (
    MODEL_ID,
    MODEL_REVISION,
    SPEC_ARTIFACT,
    TASK1_HDF5_WSL,
    build_br_xvla_training_spec,
    validate_br_xvla_training_spec,
)

DEFAULT_OUTPUT_ROOT = Path("runs/xvla_prior/epoch5_br_xvla_training")
...
# leaving phase-2 starts in the task-1 training/validation splits.  Longer
# 96-frame clips were fine for the one-target gradient smoke but erase phase-2
# candidates because both-target-in-basket states occur near demo endings.
...
def _load_spec(path: Path) -> dict[str, Any]:
    spec = json.loads(path.read_text(encoding="utf-8")) if path.exists() else build_br_xvla_training_spec()
    errors = validate_br_xvla_training_spec(spec)
    if errors:
        raise ValueError(f"invalid BR-XVLA training spec: {'; '.join(errors)}")
    return spec
...
def _arm_by_id(spec: dict[str, Any], arm_id: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise ValueError(f"arm_id {arm_id!r} is not exactly one frozen BR-XVLA arm")
    return matches[0]
...
def build_phase_clip_index(
    ...
    *,
    ...
    """Build candidate clip starts grouped by task-1 basket phase count."""
    ...
    with h5py.File(hdf5_path, "r") as handle:
        ...
        for demo_index, demo_name in enumerate(names):
            ...
            counts = target_count_in_basket(states, basket_xy_threshold)
...
def select_clip_for_step(
    ...
    *,
    ...
    raise RuntimeError("no BR-XVLA training clips are available")


def materialize_xvla_clip(source_hdf5: Path, output_dir: Path, clip: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        **clip,
        ...
        "target_count_first": int(counts[0]) if counts.size else None,
        "one_target_fraction": float(np.mean(counts == 1)) if counts.size else None,
...
def _build_official_style_optimizer(model: Any, spec: dict[str, Any], arm: dict[str, Any]) -> Any:
    ...
    if not groups:
        raise RuntimeError("no trainable BR-XVLA optimizer groups found")
    optimizer = torch.optim.AdamW(groups, betas=(0.9, 0.95))
    optimizer._br_xvla_phase_weight_lambda = float(arm["phase_weight_lambda"])  # type: ignore[attr-defined]
    return optimizer
...
def _load_model_processor_optimizer(config: TrainArmConfig, spec: dict[str, Any], arm: dict[str, Any]) -> tuple[Any, Any, Any, dict[str, Any]]:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA unavailable for BR-XVLA bounded training")
...
def run_training_arm(config: TrainArmConfig) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "schema_version": "2026-07-17.epoch5_br_xvla_bounded_training.v1",
        "method": "BR-XVLA",
        "arm_id": config.arm_id,
        "role": arm["role"],
        "status": "RUNNING",
        "success": False,
        "decision": "BR_XVLA_TRAINING_RUNNING",
        ...
        "resume_command": (
            ...
            f"-m tca_map.xvla_task1.train_lora --spec {config.spec_path.as_posix()} "
        ...
        for step_index, clip in enumerate(selected_clips):
            ...
            last_metric = {
                ...
                "materialized": {
                    ...
                    for key in ("clip_steps", "target_count_first", "one_target_fraction", "abs_action_6d_shape")
                },
                "losses": losses,
        ...
        result.update(
            {
                "status": "COMPLETE",
                "success": True,
                "decision": "BR_XVLA_BOUNDED_TRAINING_ARM_COMPLETE",
                "elapsed_seconds": float(time.monotonic() - started),
                "last_metric": last_metric,
        ...
        result.update(
            {
                "status": "FAILED",
                "success": False,
                "decision": "BR_XVLA_BOUNDED_TRAINING_ARM_FAILED",

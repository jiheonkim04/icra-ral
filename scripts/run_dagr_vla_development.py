"""Run DAGR-VLA development-only Stage 0 audit."""
...
from tca_map.smolvla.dagr_vla import PROPOSAL_HASH, audit_dagr_records, run_validation_search  # noqa: E402
...
def _write_md(path: Path, report: Mapping[str, Any]) -> None:
    probe = report.get("route_probe_summary") or {}
    train_labels = report.get("train_route_label_summary") or {}
    validation_labels = report.get("validation_route_label_summary") or {}
    target_metrics = report.get("target_delta_metrics_validation") or {}
    lines = [
        ...
        f"- validation any-route fraction: `{report['validation_any_route_fraction']}`",
        f"- full-vs-shared target mean L2: `{target_metrics.get('full_vs_shared_mean_l2')}`",
        f"- full-vs-static target mean L2: `{target_metrics.get('full_vs_static_mean_l2')}`",
        ...
        "",
        "```json",
        json.dumps(report.get("route_thresholds"), indent=2, sort_keys=True),
        "```",
        "",
        ...
        "",
        "```json",
        json.dumps(validation_labels, indent=2, sort_keys=True),
        "```",
        "",
        "Route probe summary:",
        "",
        ...
        "",
        "```json",
        json.dumps(report.get("route_pair_jaccard_validation"), indent=2, sort_keys=True),
        "```",
        "",
        "Split manifest:",
        "",
...
def _write_validation_md(path: Path, report: Mapping[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- selected residual alpha: `{selected.get('residual_alpha')}`",
        f"- selected route architecture: `{selected.get('route_architecture')}`",
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    f"{item.get('residual_alpha')}",
                    f"`{item.get('route_architecture')}`",
                    f"{score.get('route_predictability')}",
                    f"{score.get('clean_retention_and_bounded_delta')}",
                    f"{score.get('full_proxy_ablation_distinction')}",
                    f"{score.get('action_validity_and_group_delta')}",
...
def main() -> int:
    ...
    parser.add_argument("--json-output", default="reports/dagr_vla/development_audit.json")
    parser.add_argument("--md-output", default="reports/dagr_vla/development_audit.md")
    parser.add_argument("--route-label-output", default="reports/dagr_vla/route_label_manifest.json")
    parser.add_argument("--split-output", default="reports/dagr_vla/split_manifest.json")
    parser.add_argument("--validation-json-output", default="reports/dagr_vla/validation_search.json")
    parser.add_argument("--validation-md-output", default="reports/dagr_vla/validation_search.md")
    parser.add_argument("--selected-config-output", default="reports/dagr_vla/selected_config.json")
    parser.add_argument("--checkpoint-dir", default="reports/dagr_vla/validation_checkpoints")
    ...
    if args.mode in {"audit", "all"}:
        report = audit_dagr_records(artifact["records"])
        report = {
            **report,
            "date_kst": DATE_KST,
            "mode": "audit",
        ...
        _write_json(Path(args.route_label_output), report["route_label_manifest"])

param(
    ...
    [switch]$AllowWslShutdownAfterChild,
...
$StageRoot = Join-Path $RepoRoot "runs\epoch6_contact_transition_topology\stage0a"
$RunRoot = Join-Path $StageRoot $RunId
$RunRootWsl = "/mnt/c/Users/jiheo/tca_map/runs/epoch6_contact_transition_topology/stage0a/$RunId"
$HostLock = Join-Path $StageRoot "host_resource_smoke.global.lock.json"
$HostLockWsl = "/mnt/c/Users/jiheo/tca_map/runs/epoch6_contact_transition_topology/stage0a/host_resource_smoke.global.lock.json"
...
if ($AllowWslShutdownAfterChild -and $AllowWslCacheDropAfterChild) {
    throw "Choose at most one explicit WSL teardown mechanism"
}
...
try {
    ...
    while (-not $process.HasExited) {
        ...
        if (
            [double]$sample.pagefile_current_usage_mib -gt [double]$baseline.pagefile_current_usage_mib -or
            [int64]$sample.page_writes_per_sec -gt 0 -or
            [int64]$sample.pages_output_per_sec -gt 0
        ) {
            $pagefileActivityTerminated = $true
        }
        if ([double]$sample.used_fraction -gt 0.75) {
            $hostCeilingTerminated = $true
        }
        if ($pagefileActivityTerminated -or $hostCeilingTerminated) {
    ...
    if ($AllowWslShutdownAfterChild) {
        & wsl.exe --shutdown
        Start-Sleep -Seconds 4
    } elseif ($AllowWslCacheDropAfterChild) {
    ...
    $memoryReleaseVerified = [bool]([int64]$afterRelease.physical_used_bytes -le ([int64]$baseline.physical_used_bytes + 512MB))
    ...
    if ($hostCeilingTerminated) {
        $decision = "EPOCH6_CONTACT_STAGE0A_RESOURCE_SMOKE_FAIL_HOST_CEILING"
    } elseif ($pagefileActivityTerminated -or $pagefileGrowthMiB -gt 0 -or $pagefileWriteActivity) {
        $decision = "EPOCH6_CONTACT_STAGE0A_RESOURCE_SMOKE_FAIL_PAGEFILE_ACTIVITY"
    } elseif ($childExitCode -ne 0 -or -not $internalValid) {
        $decision = "EPOCH6_CONTACT_STAGE0A_RESOURCE_SMOKE_FAIL_CHILD_OR_INTERNAL"
    } elseif (-not $memoryReleaseVerified -or -not $gpuReleaseVerified) {
        $decision = "EPOCH6_CONTACT_STAGE0A_RESOURCE_SMOKE_FAIL_TEARDOWN"
    } else {
        $decision = "EPOCH6_CONTACT_STAGE0A_RESOURCE_SMOKE_PASS"
    }
    $payload = [ordered]@{
        schema_version = "epoch6.contact_topology.host_resource_smoke.v1"
        ...
        wsl_shutdown_after_child_requested = [bool]$AllowWslShutdownAfterChild
    ...
    if ($decision -eq "EPOCH6_CONTACT_STAGE0A_RESOURCE_SMOKE_PASS") { exit 0 }
    exit 1
} finally {

"""Run CALA-VLA development-only Stage 0 audit."""
...
from tca_map.smolvla.cala_vla import PROPOSAL_HASH, audit_cala_records  # noqa: E402
...
def _write_md(path: Path, report: Mapping[str, Any]) -> None:
    ...
    train_labels = report.get("train_latent_label_summary") or {}
    validation_labels = report.get("validation_latent_label_summary") or {}
    predictability = report.get("latent_predictability_summary") or {}
    gradient = report.get("gradient_audit") or {}
    lines = [
        "# CALA-VLA Development Audit",
        "",
        f"Date: `{DATE_KST}`",
        "",
        ...
        f"- future action segments used at inference: `{source_gate.get('future_action_segments_used_at_inference')}`",
        f"- latent labels used at inference: `{source_gate.get('latent_labels_used_at_inference')}`",
        f"- train latent variance nonzero dims: `{train_labels.get('latent_variance_nonzero_dims')}`",
        f"- validation latent variance nonzero dims: `{validation_labels.get('latent_variance_nonzero_dims')}`",
        f"- validation max high/low bin share: `{validation_labels.get('max_high_low_bin_share')}`",
        f"- latent predictability margin: `{predictability.get('accuracy_margin')}`",
        ...
        f"- latent gradient norm: `{gradient.get('latent_probe_gradient_norm')}`",
        ...
        "",
        "```json",
        json.dumps(report.get("latent_label_manifest"), indent=2, sort_keys=True),
        "```",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--json-output", default="reports/cala_vla/development_audit.json")
    parser.add_argument("--md-output", default="reports/cala_vla/development_audit.md")
    parser.add_argument("--source-gate-output", default="reports/cala_vla/source_gate_manifest.json")
    parser.add_argument("--latent-label-output", default="reports/cala_vla/latent_label_manifest.json")
    parser.add_argument("--split-output", default="reports/cala_vla/split_manifest.json")
    args = parser.parse_args()

    artifact = _read_json(Path(args.prediction_artifact))
    report = audit_cala_records(
        artifact["records"],
    ...
    _write_json(Path(args.latent_label_output), report["latent_label_manifest"])

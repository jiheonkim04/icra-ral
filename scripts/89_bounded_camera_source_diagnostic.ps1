param(
    ...
    [string[]]$CameraAliasStrategies = @("current_aliases", "camera3_eye_in_hand", "all_agentview"),
    [int]$TimeoutSecondsPerVariant = 1800,
    [string]$JsonReportPath = "reports\camera_source_diagnostic_report.json",
    [string]$MarkdownReportPath = "reports\camera_source_diagnostic_report.md"
)
...
Write-Host "Bounded camera-source diagnostic"
Write-Host "Repo root: $RepoRoot"
Write-Host "This script runs only a bounded one-task camera-source diagnostic. It does not download, train, run GPU jobs, execute OpenVLA-OFT, run multi-seed evaluation, or make benchmark/SOTA/paper-grade claims."
...
function Write-Reports {
    ...
    $lines = @(
        "# Camera-Source Diagnostic Report",
        "",
        "- decision: $($Report.decision)",
        "- passed: $($Report.camera_source_diagnostic_passed)",
        "- camera alias strategies: $($Report.camera_alias_strategies -join ', ')",
        "- variants completed: $($Report.result.variants_completed)",
        "- best camera alias strategy: $($Report.result.best_camera_alias_strategy)",
        ...
        "",
        "This report is camera-source diagnostic evidence only. It is not standard success, benchmark success, SOTA evidence, or paper-grade evidence."
    )
...
function New-BaseReport {
    return [ordered]@{
        policy = [ordered]@{
            camera_source_diagnostic = $true
            task_local_gate_required = "ALLOW_CAMERA_SOURCE_DIAGNOSTIC=1"
        ...
        camera_alias_strategies = @($CameraAliasStrategies)
        plan = $null
        wsl = [ordered]@{}
        variants = @()
        result = [ordered]@{
            variants_completed = 0
            best_camera_alias_strategy = $null
            best_diagnostic_success_rate = 0.0
            best_reward_sum = 0.0
        }
        camera_source_diagnostic_passed = $false
...
$allowedCameraStrategies = @("current_aliases", "camera3_eye_in_hand", "all_agentview")

$report = New-BaseReport
if ([Environment]::GetEnvironmentVariable("ALLOW_CAMERA_SOURCE_DIAGNOSTIC") -ne "1") {
    $report.reason = "ALLOW_CAMERA_SOURCE_DIAGNOSTIC=1 is required after a green camera-source diagnostic plan."
    $report.recommended_next_step = "Run scripts\88_plan_camera_source_diagnostic.ps1 first, then set ALLOW_CAMERA_SOURCE_DIAGNOSTIC=1 only for this bounded task if the plan says proceed."
    Write-Reports -Report $report
    exit 0
}
if ($dangerousGatesSet.Count -gt 0) {
    $report.reason = "camera-source diagnostic refuses unrelated execution gates: $($dangerousGatesSet -join ', ')"
    $report.recommended_next_step = "Unset broad rollout, benchmark, OpenVLA, GPU, matrix, adapter-strategy, action-scale, prompt-format, tiny-rollout, and training gates before this runner."
    Write-Reports -Report $report
    exit 0
}
if ($TaskCount -ne 1 -or $MaxStepsPerVariant -gt 10 -or $MaxStepsPerVariant -lt 1 -or $CameraAliasStrategies.Count -gt 3 -or $CameraAliasStrategies.Count -lt 1) {
    $report.reason = "camera-source runner allows exactly one task, 1..10 steps per variant, and 1..3 camera variants."
    $report.recommended_next_step = "Use TaskCount=1, MaxStepsPerVariant<=10, and no more than 3 camera alias strategies."
    Write-Reports -Report $report
    exit 0
}
foreach ($strategy in $CameraAliasStrategies) {
    if ($allowedCameraStrategies -notcontains $strategy) {
        $report.reason = "unsupported camera alias strategy: $strategy"
        $report.recommended_next_step = "Use current_aliases, camera3_eye_in_hand, or all_agentview."
        Write-Reports -Report $report
        exit 0
    }
}
if ([double]::IsNaN($ActionScale) -or [double]::IsInfinity($ActionScale) -or $ActionScale -le 0 -or $ActionScale -gt 2.0) {
    ...
    $report.recommended_next_step = "Use conservative action scale 1.0 for camera-source diagnostics."
    Write-Reports -Report $report
    exit 0
}

$planJson = Join-Path $RepoRoot "runs\camera_source_diagnostic\plan_report.json"
$planMd = Join-Path $RepoRoot "runs\camera_source_diagnostic\plan_report.md"
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $planJson) | Out-Null
$savedGate = [Environment]::GetEnvironmentVariable("ALLOW_CAMERA_SOURCE_DIAGNOSTIC")
Remove-Item Env:\ALLOW_CAMERA_SOURCE_DIAGNOSTIC -ErrorAction SilentlyContinue
try {
    & powershell -ExecutionPolicy Bypass -File (Join-Path $RepoRoot "scripts\88_plan_camera_source_diagnostic.ps1") `
        -JsonReportPath $planJson `
        -MarkdownReportPath $planMd | Out-Null
} finally {
    if ($null -ne $savedGate) { $env:ALLOW_CAMERA_SOURCE_DIAGNOSTIC = $savedGate }
}
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $planJson)) {
    $report.reason = "camera-source diagnostic planner failed"
    $report.recommended_next_step = "Fix the planner before camera-source diagnostics."
    Write-Reports -Report $report
    exit 0
}
...
$report.plan = $plan
if (-not $plan.ready_for_camera_source_diagnostic_runner) {
    $report.reason = "planner did not authorize camera-source diagnostic runner"
...
if ($null -eq (Get-Command wsl -ErrorAction SilentlyContinue)) {
    ...
    $report.recommended_next_step = "Configure WSL before camera-source diagnostics."
    Write-Reports -Report $report
    exit 0
}
...
if (-not ($repoWsl.ok -and $smolvlaWsl.ok -and $checkpointRootWsl.ok -and $hfHomeWsl.ok -and $liberoRootWsl.ok -and $robosuiteRootWsl.ok -and $liberoDataRootWsl.ok -and $pythonProbe.ok)) {
    ...
    $report.recommended_next_step = "Fix WSL path/Python readiness before camera-source diagnostics."
    Write-Reports -Report $report
    exit 0
}

$variantReports = @()
foreach ($cameraStrategy in $CameraAliasStrategies) {
    $safeStrategy = ConvertTo-SafeName -Value $cameraStrategy
    $innerReportWin = Join-Path $RepoRoot "runs\camera_source_diagnostic\$safeStrategy.json"
    $stdoutLogWin = Join-Path $RepoRoot "runs\camera_source_diagnostic\$safeStrategy.stdout.log"
    ...
    if (-not ($innerReportWsl.ok -and $stdoutLogWsl.ok)) {
        $variantReports += [ordered]@{ camera_alias_strategy = $cameraStrategy; passed = $false; reason = "failed to map WSL report/log paths" }
        continue
    }
    $bashCommand = @"
...
"@
    ...
    $variant = [ordered]@{
        camera_alias_strategy = $cameraStrategy
        prompt_strategy = $PromptStrategy
        action_adapter_strategy = $ActionAdapterStrategy
        action_scale = $ActionScale
        ...
        reward_sum = 0.0
        image_sources = @{}
        observed_camera_alias_strategy = $null
    ...
    if (Test-Path -LiteralPath $innerReportWin) {
        try {
            ...
            if ($tasks.Count -gt 0) {
                $lastTask = $tasks[-1]
                $variant.image_sources = $lastTask.last_batch_metadata.image_sources
                $variant.observed_camera_alias_strategy = $lastTask.last_batch_metadata.camera_alias_strategy
...
if ($completed.Count -gt 0) {
    ...
    $report.result.best_camera_alias_strategy = $best.camera_alias_strategy
...
$report.camera_source_diagnostic_passed = ($completed.Count -eq $CameraAliasStrategies.Count)
$report.decision = if ($report.camera_source_diagnostic_passed) { "proceed" } else { "stop" }
$report.reason = if ($report.camera_source_diagnostic_passed) {
    "bounded camera-source diagnostic completed for all requested camera alias strategies"
} else {
    "one or more camera-source diagnostic variants failed"
}
$report.recommended_next_step = if ($report.camera_source_diagnostic_passed) {
    "Summarize camera-source diagnostics and decide whether state-sufficiency diagnostics are needed. Keep evidence diagnostic only."
} else {
    "Inspect failed camera-source variants before additional diagnostics."
}

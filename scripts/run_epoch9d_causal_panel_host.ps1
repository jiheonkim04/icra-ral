...
$outputDir = Join-Path $repo "reports\epoch9d_causal_panel"
...
if ((Test-Path -LiteralPath $resultPath) -and -not $Resume) {
    throw "A causal result already exists; use -Resume for missing-key-only continuation"
}
New-Item -ItemType Directory -Path $outputDir -Force | Out-Null

$existing = & wsl.exe -e bash -lc "pgrep -af '[r]un_epoch9d_causal_panel.py' || true"
if ($existing) {
    throw "An Epoch 9D causal runner is already active: $existing"
}
...
$bashCommand = "set -e; cd /mnt/c/Users/jiheo/tca_map; export MUJOCO_GL=egl; export PYTHONUNBUFFERED=1; exec /home/jiheon/miniconda3-official/envs/official-smolvla-libero/bin/python scripts/run_epoch9d_causal_panel.py --mode $Mode$resumeArgument"
...
while (-not $process.HasExited) {
    ...
    if ($sample.host_ram_percent -ge $hostCeilingPercent) {
        ...
        $terminateCommand = 'pids=$(pgrep -f ''^/home/jiheon/miniconda3-official/envs/official-smolvla-libero/bin/python scripts/run_epoch9d_causal_panel.py'' || true); if [ -n "$pids" ]; then kill -TERM $pids; fi'
...
if ($terminationRequested -and -not $process.HasExited) {
    try {
        ...
        $killCommand = 'pids=$(pgrep -f ''^/home/jiheon/miniconda3-official/envs/official-smolvla-libero/bin/python scripts/run_epoch9d_causal_panel.py'' || true); if [ -n "$pids" ]; then kill -KILL $pids; fi'
...
$monitor = [ordered]@{
    schema_version = "epoch9d.causal_host_resource_monitor.v1"
    ...
    scientific_result_path = "reports/epoch9d_causal_panel/result.json"
    scientific_result_sha256_after_runner = $resultHash
    stdout_path = "reports/epoch9d_causal_panel/runner_stdout.log"
    stdout_sha256 = if (Test-Path -LiteralPath $stdoutPath) { (Get-FileHash -Algorithm SHA256 -LiteralPath $stdoutPath).Hash } else { $null }
    stderr_path = "reports/epoch9d_causal_panel/runner_stderr.log"

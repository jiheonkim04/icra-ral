...
RUN_ROOT="${RUN_ROOT:-runs/marc_vla_stage_a}"
PARTIAL_RESULT="reports/marc_vla/stage_a_partial_result.json"
FINAL_RESULT="reports/marc_vla/stage_a_result.json"
...
resume_command="cd ${REPO_ROOT} && ${PYTHON_BIN} scripts/run_marc_vla_stage_a.py --mode stage-a"
printf '%s\n' "$resume_command" > "${run_dir}/resume_command.txt"

partial_count() {
  "$PYTHON_BIN" -c 'import json, pathlib
p = pathlib.Path("reports/marc_vla/stage_a_partial_result.json")
...
"$python_bin" scripts/run_marc_vla_stage_a.py --mode stage-a &
child_pid=$!
...
exit "$code"
' marc_stage_a "$REPO_ROOT" "$PYTHON_BIN" "$run_dir" > "${run_dir}/stdout.log" 2> "${run_dir}/stderr.log" < /dev/null &

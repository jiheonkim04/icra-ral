"""Run the frozen CFR-VLA Stage 0 continuous refinement development audit."""
...
from tca_map.smolvla.cfr_vla import (  # noqa: E402
    ACTION_DIM,
    CHUNK_SIZE,
    DFM_BINS,
    DFM_HEADROOM_ABSOLUTE_HUBER_GATE,
    DFM_HEADROOM_RELATIVE_GATE,
    PHASE_BINS,
    PROPRIO_DIM,
    PROPOSAL_HASH,
    REFINEMENT_STEPS,
    ...
    predict_single_step_refinement,
    prediction_metrics,
...
POLICY_PROBE = "cfr_stage0_continuous_full_chunk_refinement"
SEED = 20262600
FLOW_TIME = 0.5
DISCOVERY_ROWS_PER_TASK = 128
VALIDATION_ROWS_PER_TASK = 32
GRADIENT_RATIO_MAX = 100.0
PROPOSAL_FILE = REPO_ROOT / "reports" / "cfr_vla" / "researcher_proposal.md"
PROPOSAL_HASH_FILE = REPO_ROOT / "reports" / "cfr_vla" / "proposal_hash.txt"
...
def _serializer_preflight(path: Path) -> dict[str, Any]:
    manifest_row = {
        ...
        "proxy_variant": "cfr_full",
        "policy_probe": POLICY_PROBE,
    }
    manifest_row["row_key"] = cfr_row_key(manifest_row)
    fixture = {
        "method": "CFR-VLA",
        ...
        "refinement_diagnostic": {
            "base_to_expert_residual_norm": np.float32(1.0),
            "dfm_proxy_huber": np.float32(0.5),
            "cfr_proxy_huber": np.float32(0.25),
        },
    ...
    if not result["passed"]:
        raise RuntimeError("CFR serializer preflight hash did not reproduce")
    return result


def _official_prior_asset_check(path: Path) -> dict[str, Any]:
    candidates = {
        "official_repository": REPO_ROOT / "third_party" / "DFM-VLA",
        "asset_repository": _asset_path("DFM-VLA"),
        "checkpoint_dir": _asset_path("checkpoints", "dfm_vla"),
        "inference_code": _asset_path("dfm_vla", "inference"),
        "training_code": _asset_path("dfm_vla", "training"),
    }
    ...
    label = "dfm_vla_official" if official_ready else "dfm_vla_continuous_refinement_proxy"
    deviations = [] if official_ready else [
        "official DFM-VLA repository/checkpoint/inference assets are not all locally verified",
        "Stage 0 fixes policy 2 as a transparent iterative full-sequence refinement proxy until official assets are installed",
    ]
    result = {
        "method": "CFR-VLA",
        "closest_prior": "DFM-VLA",
...
def _evenly_spaced(rows: Sequence[dict[str, Any]], count: int) -> list[dict[str, Any]]:
    ...
    if len(indices) != len(set(indices)):
        raise RuntimeError("deterministic CFR sampler produced duplicate indices")
...
def _build_manifest(data_root: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        with h5py.File(source, "r") as handle:
            ...
            for demo_id in range(10):
                ...
                for frame in range(max(0, valid_count)):
                    ...
                    row["feature_key"] = cfr_feature_key(row)
    ...
    for group in sorted(groups):
        ...
        if len(selected) != target_count:
            raise RuntimeError(f"CFR manifest group {group} has {len(selected)} rows, expected {target_count}")
...
def _load_feature(path: Path) -> np.ndarray:
    ...
    if feature.shape != (VISUAL_FEATURE_DIM,) or not np.isfinite(feature).all():
        raise RuntimeError(f"invalid CFR visual feature {path}: {feature.shape}")
    return feature
...
def _base_chunk_path(base_chunk_dir: Path, row_key: str) -> Path:
    digest = hashlib.sha256(row_key.encode("utf-8")).hexdigest().upper()
    return base_chunk_dir / f"{digest}.npz"
...
def _load_base_chunk(path: Path) -> np.ndarray:
    with np.load(path, allow_pickle=False) as loaded:
        chunk = np.asarray(loaded["base_chunk"], dtype=np.float32)
    if chunk.shape != (CHUNK_SIZE, ACTION_DIM) or not np.isfinite(chunk).all():
        raise RuntimeError(f"invalid CFR base chunk {path}: {chunk.shape}")
    return chunk


def _load_or_decode_base_chunk(
    policy: Any,
    preprocessor: Any,
    postprocessor: Any,
    ...
    path = _base_chunk_path(paths["base_chunk_dir"], str(row["row_key"]))
    if path.is_file():
    ...
    noise = _noise(str(row["row_key"]), "base_decode", shape, "cuda")
...
def _proprio_from_obs(observations: Any, frame_index: int) -> np.ndarray:
    ...
    if proprio.shape != (PROPRIO_DIM,) or not np.isfinite(proprio).all():
        raise ValueError(f"CFR proprio must have shape [{PROPRIO_DIM}], received {proprio.shape}")
    return proprio
...
def _partial_row(
    ...
    return {
        ...
        "refinement_steps": int(row["refinement_steps"]),
...
def _load_resume(path: Path, manifest_rows: Sequence[Mapping[str, Any]], manifest_hash: str) -> tuple[list[dict[str, Any]], int, str | None]:
    ...
    if partial.get("method") != "CFR-VLA" or partial.get("proposal_hash") != PROPOSAL_HASH:
        raise RuntimeError("partial result identity does not match frozen CFR proposal/manifest")
    rows = list(partial.get("rows") or [])
    if partial.get("manifest_hash") is None and not rows:
        return [], 0, None
    if partial.get("manifest_hash") != manifest_hash:
        raise RuntimeError("partial result identity does not match frozen CFR manifest")
...
def _materialize_arrays(manifest: Sequence[Mapping[str, Any]], partial_rows: Sequence[Mapping[str, Any]]) -> dict[str, np.ndarray]:
    ...
    for row in manifest:
        ...
        raw_feature = raw_refinement_feature(visual, proprio, int(row["task_index"]), float(row["phase"]))
...
def _fit_cfr_models(
    ...
    residual_baseline = task_phase_mean_chunks(
        arrays["residual_chunk"][discovery],
    ...
    task_phase_prediction = arrays["base_chunk"][validation] + residual_baseline
    task_phase_metrics = prediction_metrics(
        flattened_chunks(task_phase_prediction),
        flattened_chunks(arrays["base_chunk"][validation]),
        flattened_chunks(arrays["chunk"][validation]),
    )

    dfm_model = fit_dfm_proxy(
    ...
    dfm_prediction = predict_dfm_proxy(
        dfm_model,
    ...
    cfr_model = fit_iterative_refinement(
        zfeatures[discovery],
        arrays["base_chunk"][discovery],
        arrays["chunk"][discovery],
        steps=REFINEMENT_STEPS,
    )
    cfr_prediction = predict_iterative_refinement(
        cfr_model,
        zfeatures[validation],
        arrays["base_chunk"][validation],
    )

    no_iter_model = fit_single_step_refinement(
        zfeatures[discovery],
        arrays["base_chunk"][discovery],
        arrays["chunk"][discovery],
    )
    no_iter_prediction = predict_single_step_refinement(
        no_iter_model,
        zfeatures[validation],
        arrays["base_chunk"][validation],
    )

    arrays["dfm_proxy_chunk"] = np.zeros_like(arrays["chunk"], dtype=np.float64)
    arrays["cfr_prediction_chunk"] = np.zeros_like(arrays["chunk"], dtype=np.float64)
    arrays["no_iterative_prediction_chunk"] = np.zeros_like(arrays["chunk"], dtype=np.float64)
    arrays["task_phase_prediction_chunk"] = np.zeros_like(arrays["chunk"], dtype=np.float64)
    arrays["dfm_proxy_chunk"][validation_indices] = dfm_prediction
    arrays["cfr_prediction_chunk"][validation_indices] = cfr_prediction
    arrays["no_iterative_prediction_chunk"][validation_indices] = no_iter_prediction
    arrays["task_phase_prediction_chunk"][validation_indices] = task_phase_prediction

    residual_metrics = prediction_metrics(
        flattened_chunks(cfr_prediction),
        flattened_chunks(task_phase_prediction),
        flattened_chunks(arrays["chunk"][validation]),
    )
    headroom_metrics = prediction_metrics(
        flattened_chunks(cfr_prediction),
        flattened_chunks(dfm_prediction),
        flattened_chunks(arrays["chunk"][validation]),
    )
    no_iter_metrics = prediction_metrics(
        flattened_chunks(cfr_prediction),
        flattened_chunks(no_iter_prediction),
        flattened_chunks(arrays["chunk"][validation]),
    )
    residual_variance = np.var(arrays["residual_chunk"][discovery].reshape((-1, ACTION_DIM)), axis=0)
    cfr_no_iter_delta = flattened_chunks(cfr_prediction) - flattened_chunks(no_iter_prediction)
    cfr_dfm_delta = flattened_chunks(cfr_prediction) - flattened_chunks(dfm_prediction)

    models = {
        "zscore": zscore,
        "dfm_proxy": dfm_model,
        "cfr_iterative": cfr_model,
        "cfr_no_iterative": no_iter_model,
    ...
    audit = {
        ...
        "refinement_steps": REFINEMENT_STEPS,
        "dfm_bins": DFM_BINS,
        "task_phase_baseline": task_phase_metrics,
        "residual_probe": residual_metrics,
        "dfm_proxy_headroom": headroom_metrics,
        "no_iterative_ablation": no_iter_metrics,
        "base_to_expert_residual_variance_by_action_dim": residual_variance,
        "base_to_expert_residual_variance_all_positive": bool(np.all(residual_variance > 0.0)),
        "base_to_expert_residual_huber": mean_huber(arrays["base_chunk"][validation], arrays["chunk"][validation]),
        "dfm_proxy_huber": mean_huber(dfm_prediction, arrays["chunk"][validation]),
        "cfr_prediction_huber": mean_huber(cfr_prediction, arrays["chunk"][validation]),
        "no_iterative_prediction_huber": mean_huber(no_iter_prediction, arrays["chunk"][validation]),
        "iterative_cfr_no_iter_delta_norm_mean": float(np.mean(np.linalg.norm(cfr_no_iter_delta, axis=1))),
        "iterative_cfr_distinct_from_no_iterative": bool(float(np.mean(np.linalg.norm(cfr_no_iter_delta, axis=1))) > 1e-12),
        "cfr_dfm_delta_norm_mean": float(np.mean(np.linalg.norm(cfr_dfm_delta, axis=1))),
        "minimum_discovery_row_count": int(len(discovery_indices)),
        "minimum_validation_row_count": int(len(validation_indices)),
    }
...
def _data_summary(
    ...
    headroom = model_audit["dfm_proxy_headroom"]
    ...
    return {
        ...
        "base_to_expert_residual_variance_all_positive": bool(
            model_audit["base_to_expert_residual_variance_all_positive"]
        ),
        "base_to_expert_residual_variance_by_action_dim": model_audit[
            "base_to_expert_residual_variance_by_action_dim"
        ],
        "base_to_expert_residual_huber": model_audit["base_to_expert_residual_huber"],
        ...
        "dfm_proxy_headroom_relative_improvement": headroom["relative_mse_improvement"],
        "dfm_proxy_headroom_absolute_huber_improvement": headroom["absolute_huber_improvement"],
        "dfm_headroom_relative_gate": DFM_HEADROOM_RELATIVE_GATE,
        "dfm_headroom_absolute_huber_gate": DFM_HEADROOM_ABSOLUTE_HUBER_GATE,
        "iterative_cfr_distinct_from_no_iterative": bool(
            model_audit["iterative_cfr_distinct_from_no_iterative"]
        ),
        "iterative_cfr_no_iter_delta_norm_mean": model_audit["iterative_cfr_no_iter_delta_norm_mean"],
        "cfr_dfm_delta_norm_mean": model_audit["cfr_dfm_delta_norm_mean"],
        "cfr_prediction_huber": model_audit["cfr_prediction_huber"],
        "dfm_proxy_huber": model_audit["dfm_proxy_huber"],
        "no_iterative_prediction_huber": model_audit["no_iterative_prediction_huber"],
...
def _action_stats(postprocessor: Any) -> dict[str, Any]:
    for step in postprocessor.steps:
        ...
        if "mean" not in stats or "std" not in stats:
            raise RuntimeError("CFR requires checkpoint MEAN_STD action statistics")
...
def _write_action_semantics(path: Path, action_stats: Mapping[str, Any]) -> dict[str, Any]:
    semantics = {
        ...
        "same_definition_applies_to_policies": [
            "smolvla_base",
            "dfm_vla_continuous_refinement_proxy",
            "cfr_full",
...
def _cfr_loss(
    ...
    cfr_target_loss = functional.smooth_l1_loss(clean_action, target_native, beta=1.0)
    clean_loss = functional.smooth_l1_loss(clean_action, batch["action"][:, :CHUNK_SIZE, :ACTION_DIM], beta=1.0)
    total = flow_loss + cfr_target_loss + 0.1 * clean_loss
    return total, flow_loss, cfr_target_loss, clean_loss
...
def _gradient_audit(
    ...
    if not named or not all("lora_" in name.lower() for name, _ in named):
        raise RuntimeError("CFR expected only LoRA trainable parameters")
    ...
    target = np.asarray(arrays["cfr_prediction_chunk"][row_index], dtype=np.float64)
    policy.zero_grad(set_to_none=True)
    cfr_loss, flow_component, target_component, clean_component = _cfr_loss(
        policy, batch, noise, time_value, action_stats, target
    )
    cfr_gradients, cfr_norm, cfr_finite = _gradient_values(cfr_loss, named)
    cfr_value = float(cfr_loss.detach().item())
    target_value = float(target_component.detach().item())
    clean_value = float(clean_component.detach().item())
    dot = 0.0
    for flow_gradient, cfr_gradient in zip(flow_gradients, cfr_gradients, strict=True):
        if flow_gradient is not None and cfr_gradient is not None:
            dot += float(torch.sum(flow_gradient * cfr_gradient).item())
    cosine = dot / max(flow_norm * cfr_norm, 1e-12)
    ...
    return {
        ...
        "cfr_gradient_norm": cfr_norm,
        "cfr_to_flow_gradient_ratio": cfr_norm / max(flow_norm, 1e-12),
        "gradient_cosine": cosine,
        "flow_gradient_finite_fraction": flow_finite,
        "cfr_gradient_finite_fraction": cfr_finite,
        "frozen_parameter_gradient_count": len(frozen_gradient_names),
        "frozen_parameter_gradient_names": frozen_gradient_names,
        "cfr_gradient_nonzero": cfr_norm > 0.0,
        "finite_objectives_and_gradients": bool(
            np.isfinite([flow_value, cfr_value, target_value, clean_value, flow_norm, cfr_norm, cosine]).all()
            and flow_finite == 1.0
            and cfr_finite == 1.0
        ),
...
def _identity_and_gradient_audit(
    ...
    cfr_target = np.asarray(arrays["cfr_prediction_chunk"][row_index], dtype=np.float64)
    dfm_target = np.asarray(arrays["dfm_proxy_chunk"][row_index], dtype=np.float64)
    projection_delta = cfr_target - dfm_target
    ...
    identity = {
        ...
        "ours_action_diagnostic": cfr_target[: min(3, len(cfr_target))].tolist(),
        "dfm_proxy_action_diagnostic": dfm_target[: min(3, len(dfm_target))].tolist(),
        ...
        "cfr_residual_norm": residual_norm,
        "cfr_gate_value": 1.0,
        ...
        "training_only_cfr_absent_from_policy_parameters": not any(
            "cfr" in name.lower() for name, _ in reloaded.named_parameters()
        ),
    }
    del reloaded
    gc.collect()
...
def _decision_inputs(
    ...
    return Stage0DecisionInputs(
        ...
        base_to_expert_residual_variance_all_positive=bool(data["base_to_expert_residual_variance_all_positive"]),
        ...
        dfm_proxy_headroom_relative_improvement=float(data["dfm_proxy_headroom_relative_improvement"]),
        dfm_proxy_headroom_absolute_huber_improvement=float(data["dfm_proxy_headroom_absolute_huber_improvement"]),
        iterative_cfr_distinct_from_no_iterative=bool(data["iterative_cfr_distinct_from_no_iterative"]),
        finite_objectives_and_gradients=bool(gradient.get("finite_objectives_and_gradients", False)),
        cfr_gradient_nonzero=bool(gradient.get("cfr_gradient_nonzero", False)),
        gradient_ratio_at_most_100=float(gradient.get("cfr_to_flow_gradient_ratio", float("inf"))) <= GRADIENT_RATIO_MAX,
...
def _result_markdown(result: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# CFR-VLA Stage 0 Result",
            "",
            f"Final decision: `{result['final_decision']}`.",
            "",
            f"Rows: `{result['completed_model_row_count']} / {result['planned_model_row_count']}`.",
            f"DFM-VLA prior label: `{result['official_prior_asset_check']['policy_2_label']}`.",
            f"Base-to-expert residual Huber: `{data['base_to_expert_residual_huber']}`.",
            f"CFR residual-probe relative / absolute Huber gain: `{data['residual_probe_relative_improvement']} / {data['residual_probe_absolute_huber_improvement']}`.",
            f"CFR minus DFM-proxy relative / absolute Huber gain: `{data['dfm_proxy_headroom_relative_improvement']} / {data['dfm_proxy_headroom_absolute_huber_improvement']}`.",
            f"Iterative CFR distinct from no-iterative: `{data['iterative_cfr_distinct_from_no_iterative']}`.",
            f"Flow / CFR gradient norm: `{gradient.get('flow_gradient_norm')} / {gradient.get('cfr_gradient_norm')}`.",
...
def _adjudication_markdown(result: Mapping[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "",
            "The frozen Stage 0 gates were applied without changing task sources, memory construction, DFM proxy definition, CFR iterative refinement path, baselines, or thresholds.",
            "",
        ]
    )
...
def _preflight(paths: Mapping[str, Path], started_unix: float) -> dict[str, Any]:
    ...
    for worker in _active_linux_workers():
        ...
        if "run_cfr_vla_stage0.py" not in command:
            continue
    ...
    return {
        ...
        "active_cfr_linux_workers": workers,
...
def run(args: argparse.Namespace, paths: Mapping[str, Path], state: dict[str, Any]) -> dict[str, Any]:
    ...
    if not preflight["passed"]:
        raise RuntimeError(f"CFR Stage 0 preflight failed: {preflight}")
    ...
    if not serializer_ok:
        raise RuntimeError("foreground CFR serializer preflight is absent or invalid")
    ...
    if not proposal_ok:
        raise RuntimeError("frozen CFR proposal hash mismatch")
    ...
    if canonical_json_sha256(parsed_without_hash) != manifest_hash:
        raise RuntimeError("persisted CFR manifest hash did not reproduce")
    ...
    for row in rows:
        ...
        if len(partial_rows) % 16 == 0 or len(partial_rows) == len(rows):
            print(f"[cfr-stage0] rows {len(partial_rows)}/{len(rows)}", flush=True)
    ...
    state["phase"] = "fit_cfr_audit_models"
    arrays = _materialize_arrays(rows, partial_rows)
    models, model_audit, arrays = _fit_cfr_models(rows, arrays)
    ...
    result = {
        ...
        "bounded_validation_allowed": decision == "CFR_STAGE_0_PASS_TO_BOUNDED_VALIDATION",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--report-root", default=str(REPO_ROOT / "reports" / "cfr_vla"))
    parser.add_argument("--run-root", default=str(REPO_ROOT / "runs" / "cfr_vla" / "stage0"))
    ...
    if args.serializer_preflight:
        ...
        print(f"CFR serializer preflight passed: {paths['serializer_preflight']} {result['fixture_hash']}")
        return 0
    ...
    try:
        ...
        print(f"CFR Stage 0 failed: {type(exc).__name__}: {exc}", file=sys.stderr)
        traceback.print_exc()

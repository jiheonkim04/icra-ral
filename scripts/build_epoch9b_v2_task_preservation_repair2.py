#!/usr/bin/env python3
"""Freeze fresh evidence for the inward-contact Epoch 9B repair."""
...
BASE = ROOT / "reports/epoch9b_v2_task_preservation_protocol_repair1.json"
PARENT_DEVELOPMENT = (
    ROOT / "reports/epoch9b_dynamic_nudge/development/d17_centered_contact_edge_stress/result.json"
)
OUTPUT = ROOT / "reports/epoch9b_v2_task_preservation_protocol_repair2.json"
...
def fresh_manifest() -> list[dict[str, Any]]:
    ...
    for index in range(24):
        ...
        rows.append(
            {
                "scene_id": f"epoch9b_repair2_scene_{index:03d}",
                "partition": "DEVELOPMENT_FEASIBILITY_REPAIR2",
                "source_state_demo_index": 34,
                "generator_seed": 912000 + index,
                ...
                "candidate_initial_xy_m": {
                    "front": [
                        0.081 + 0.0044 * ((5 * index + 2) % 16),
                        0.124 + 0.0033 * ((7 * index + 4) % 13),
                    ],
                    "back": [
                        -0.174 + 0.0046 * ((9 * index + 1) % 16),
                        0.026 + 0.0036 * ((5 * index + 2) % 15),
                    ],
                },
...
def build() -> dict[str, Any]:
    ...
    if development["summary"]["lane_reachability_pass_probes"] >= development["summary"]["probe_count"]:
        raise RuntimeError("repair2 requires a documented repair1 lane failure")
    manifest = fresh_manifest()
    prior_ids = {row["scene_id"] for row in protocol["feasibility_manifest"]}
    protocol.update(
        {
            "schema_version": "epoch9b.v2_task_preservation.repair2.v1",
            "status": "FROZEN_BEFORE_REPAIR2_CONTROLLER_OUTCOME",
            "frozen_at": datetime.now().astimezone().isoformat(timespec="seconds"),
            "parent_protocol_path": str(BASE.relative_to(ROOT)).replace("\\", "/"),
            "parent_protocol_sha256": sha256(BASE),
            "parent_repair1_development_no_go": {
                "path": str(PARENT_DEVELOPMENT.relative_to(ROOT)).replace("\\", "/"),
                "sha256": sha256(PARENT_DEVELOPMENT),
                "reason": "centered contact exceeded the front y-lane ceiling in three of four near-boundary stress probes",
                "summary": development["summary"],
            },
            "repair_scope": "after fixture-clear transit, select the front contact side from RGB-estimated y relative to the fixed lane midpoint so the incidental lateral response points inward; preserve impulse, response threshold, lanes, displacement rule, and GO thresholds",

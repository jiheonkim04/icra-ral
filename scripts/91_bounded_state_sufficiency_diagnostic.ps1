param(
    ...
    [string]$PromptStrategy = "bddl_language",
    [string]$CameraAliasStrategy = "current_aliases",
    [string[]]$StateAdapterStrategies = @("eef_pos_quat_first3", "eef_pos_quat_last3", "eef_pos_zero_rot"),
    [int]$TimeoutSecondsPerVariant = 1800,
    [string]$JsonReportPath = "reports\state_sufficiency_diagnostic_report.json",
    [string]$MarkdownReportPath = "reports\state_sufficiency_diagnostic_report.md"
)
...
Write-Host "Bounded state-sufficiency diagnostic"
Write-Host "Repo root: $RepoRoot"
Write-Host "This script runs only a bounded one-task state-sufficiency diagnostic. It does not download, train, run GPU jobs, execute OpenVLA-OFT, run multi-seed evaluation, or make benchmark/SOTA/paper-grade claims."
...
function Write-Reports {
    ...
    $json | Set-Content -LiteralPath $jsonFullPath -Encoding UTF8
    $lines = @(
        "# State-Sufficiency Diagnostic Report",
        "",
        "- decision: $($Report.decision)",
        "- passed: $($Report.state_sufficiency_diagnostic_passed)",
        "- state adapter strategies: $($Report.state_adapter_strategies -join ', ')",
        "- variants completed: $($Report.result.variants_completed)",
        "- best state adapter strategy: $($Report.result.best_state_adapter_strategy)",
        ...
        "",
        "This report is state-sufficiency diagnostic evidence only. It is not standard success, benchmark success, SOTA evidence, or paper-grade evidence."
    )
...
function New-BaseReport {
    return [ordered]@{
        policy = [ordered]@{
            state_sufficiency_diagnostic = $true
            task_local_gate_required = "ALLOW_STATE_SUFFICIENCY_DIAGNOSTIC=1"
        ...
        camera_alias_strategy = $CameraAliasStrategy
        state_adapter_strategies = @($StateAdapterStrategies)
        plan = $null
        wsl = [ordered]@{}
        variants = @()
        result = [ordered]@{
            variants_completed = 0
            best_state_adapter_strategy = $null
            best_diagnostic_success_rate = 0.0
            best_reward_sum = 0.0
        }
        state_sufficiency_diagnostic_passed = $false
...
$allowedStateStrategies = @("eef_pos_quat_first3", "eef_pos_quat_last3", "eef_pos_zero_rot")

$report = New-BaseReport
if ([Environment]::GetEnvironmentVariable("ALLOW_STATE_SUFFICIENCY_DIAGNOSTIC") -ne "1") {
    $report.reason = "ALLOW_STATE_SUFFICIENCY_DIAGNOSTIC=1 is required after a green state-sufficiency diagnostic plan."
    $report.recommended_next_step = "Run scripts\90_plan_state_sufficiency_diagnostic.ps1 first, then set ALLOW_STATE_SUFFICIENCY_DIAGNOSTIC=1 only for this bounded task if the plan says proceed."
    Write-Reports -Report $report
    exit 0
}
if ($dangerousGatesSet.Count -gt 0) {
    $report.reason = "state-sufficiency diagnostic refuses unrelated execution gates: $($dangerousGatesSet -join ', ')"
    $report.recommended_next_step = "Unset broad rollout, benchmark, OpenVLA, GPU, matrix, adapter-strategy, action-scale, prompt-format, camera-source, tiny-rollout, and training gates before this runner."
    Write-Reports -Report $report
    exit 0
}
if ($TaskCount -ne 1 -or $MaxStepsPerVariant -gt 10 -or $MaxStepsPerVariant -lt 1 -or $StateAdapterStrategies.Count -gt 3 -or $StateAdapterStrategies.Count -lt 1) {
    $report.reason = "state-sufficiency runner allows exactly one task, 1..10 steps per variant, and 1..3 state variants."
    $report.recommended_next_step = "Use TaskCount=1, MaxStepsPerVariant<=10, and no more than 3 state adapter strategies."
    Write-Reports -Report $report
    exit 0
}
foreach ($strategy in $StateAdapterStrategies) {
    if ($allowedStateStrategies -notcontains $strategy) {
        $report.reason = "unsupported state adapter strategy: $strategy"
        $report.recommended_next_step = "Use eef_pos_quat_first3, eef_pos_quat_last3, or eef_pos_zero_rot."
        Write-Reports -Report $report
        exit 0
    }
}
if ([double]::IsNaN($ActionScale) -or [double]::IsInfinity($ActionScale) -or $ActionScale -le 0 -or $ActionScale -gt 2.0) {
    ...
    $report.recommended_next_step = "Use conservative action scale 1.0 for state-sufficiency diagnostics."
    Write-Reports -Report $report
    exit 0
}

$planJson = Join-Path $RepoRoot "runs\state_sufficiency_diagnostic\plan_report.json"
$planMd = Join-Path $RepoRoot "runs\state_sufficiency_diagnostic\plan_report.md"
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $planJson) | Out-Null
$savedGate = [Environment]::GetEnvironmentVariable("ALLOW_STATE_SUFFICIENCY_DIAGNOSTIC")
Remove-Item Env:\ALLOW_STATE_SUFFICIENCY_DIAGNOSTIC -ErrorAction SilentlyContinue
try {
    & powershell -ExecutionPolicy Bypass -File (Join-Path $RepoRoot "scripts\90_plan_state_sufficiency_diagnostic.ps1") `
        -JsonReportPath $planJson `
        -MarkdownReportPath $planMd | Out-Null
} finally {
    if ($null -ne $savedGate) { $env:ALLOW_STATE_SUFFICIENCY_DIAGNOSTIC = $savedGate }
}
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $planJson)) {
    $report.reason = "state-sufficiency diagnostic planner failed"
    $report.recommended_next_step = "Fix the planner before state-sufficiency diagnostics."
    Write-Reports -Report $report
    exit 0
}
...
$report.plan = $plan
if (-not $plan.ready_for_state_sufficiency_diagnostic_runner) {
    $report.reason = "planner did not authorize state-sufficiency diagnostic runner"
...
if ($null -eq (Get-Command wsl -ErrorAction SilentlyContinue)) {
    ...
    $report.recommended_next_step = "Configure WSL before state-sufficiency diagnostics."
    Write-Reports -Report $report
    exit 0
}
...
if (-not ($repoWsl.ok -and $smolvlaWsl.ok -and $checkpointRootWsl.ok -and $hfHomeWsl.ok -and $liberoRootWsl.ok -and $robosuiteRootWsl.ok -and $liberoDataRootWsl.ok -and $pythonProbe.ok)) {
    ...
    $report.recommended_next_step = "Fix WSL path/Python readiness before state-sufficiency diagnostics."
    Write-Reports -Report $report
    exit 0
}

$variantReports = @()
foreach ($stateStrategy in $StateAdapterStrategies) {
    $safeStrategy = ConvertTo-SafeName -Value $stateStrategy
    $innerReportWin = Join-Path $RepoRoot "runs\state_sufficiency_diagnostic\$safeStrategy.json"
    $stdoutLogWin = Join-Path $RepoRoot "runs\state_sufficiency_diagnostic\$safeStrategy.stdout.log"
    ...
    if (-not ($innerReportWsl.ok -and $stdoutLogWsl.ok)) {
        $variantReports += [ordered]@{ state_adapter_strategy = $stateStrategy; passed = $false; reason = "failed to map WSL report/log paths" }
        continue
    }
    $bashCommand = @"
...
"@
    ...
    $variant = [ordered]@{
        state_adapter_strategy = $stateStrategy
        prompt_strategy = $PromptStrategy
        camera_alias_strategy = $CameraAliasStrategy
        action_adapter_strategy = $ActionAdapterStrategy
        action_scale = $ActionScale
        ...
        reward_sum = 0.0
        observed_state_adapter_strategy = $null
        state_adapter = $null
    ...
    if (Test-Path -LiteralPath $innerReportWin) {
        try {
            ...
            if ($tasks.Count -gt 0) {
                $lastTask = $tasks[-1]
                $variant.state_adapter = $lastTask.last_batch_metadata.state_adapter
                $variant.observed_state_adapter_strategy = $lastTask.last_batch_metadata.state_adapter_strategy
...
if ($completed.Count -gt 0) {
    ...
    $report.result.best_state_adapter_strategy = $best.state_adapter_strategy
...
$report.state_sufficiency_diagnostic_passed = ($completed.Count -eq $StateAdapterStrategies.Count)
$report.decision = if ($report.state_sufficiency_diagnostic_passed) { "proceed" } else { "stop" }
$report.reason = if ($report.state_sufficiency_diagnostic_passed) {
    "bounded state-sufficiency diagnostic completed for all requested state adapter strategies"
} else {
    "one or more state-sufficiency diagnostic variants failed"
}
$report.recommended_next_step = if ($report.state_sufficiency_diagnostic_passed) {
    "Summarize state-sufficiency diagnostics and decide whether to pivot to dataset/interface verification or no-go analysis. Keep evidence diagnostic only."
} else {
    "Inspect failed state-sufficiency variants before additional diagnostics."
}

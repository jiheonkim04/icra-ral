param(
    ...
    [string]$JsonReportPath = "reports\adapter_strategy_action_scale_diagnostic_report.json",
    [string]$MarkdownReportPath = "reports\adapter_strategy_action_scale_diagnostic_report.md"
)
...
Write-Host "Bounded adapter-strategy diagnostic"
Write-Host "Repo root: $RepoRoot"
Write-Host "This script runs only a bounded one-task adapter-strategy diagnostic. It does not download, train, run GPU jobs, execute OpenVLA-OFT, run multi-seed evaluation, or make benchmark/SOTA/paper-grade claims."
...
function Write-Reports {
    ...
    $Report | ConvertTo-Json -Depth 18 | Set-Content -LiteralPath $jsonFullPath -Encoding UTF8
    $lines = @(
        "# Adapter-Strategy Diagnostic Report",
        "",
        "- decision: $($Report.decision)",
        "- passed: $($Report.adapter_strategy_diagnostic_passed)",
        ...
        "",
        "This report is adapter-strategy diagnostic evidence only. It is not standard success, benchmark success, SOTA evidence, or paper-grade evidence."
    )
...
function New-BaseReport {
    return [ordered]@{
        policy = [ordered]@{
            adapter_strategy_diagnostic = $true
            task_local_gate_required = "ALLOW_ADAPTER_STRATEGY_DIAGNOSTIC=1"
        ...
        adapter_strategy_diagnostic_passed = $false
...
if ([Environment]::GetEnvironmentVariable("ALLOW_ADAPTER_STRATEGY_DIAGNOSTIC") -ne "1") {
    $report.reason = "ALLOW_ADAPTER_STRATEGY_DIAGNOSTIC=1 is required after a green adapter-strategy diagnostics plan."
    $report.recommended_next_step = "Run scripts\82_plan_adapter_strategy_action_scale_diagnostics.ps1 first, then set ALLOW_ADAPTER_STRATEGY_DIAGNOSTIC=1 only for this bounded task if the plan says proceed."
    Write-Reports -Report $report
    exit 0
}
if ($dangerousGatesSet.Count -gt 0) {
    $report.reason = "adapter-strategy diagnostic refuses unrelated execution gates: $($dangerousGatesSet -join ', ')"
    $report.recommended_next_step = "Unset broad rollout, benchmark, OpenVLA, GPU, matrix, tiny-rollout, and training gates before this runner."
    Write-Reports -Report $report
    exit 0
}
if ($TaskCount -ne 1 -or $MaxStepsPerVariant -gt 10 -or $MaxStepsPerVariant -lt 1 -or $Strategies.Count -gt 3 -or $Strategies.Count -lt 1) {
    $report.reason = "adapter-strategy runner allows exactly one task, 1..10 steps per variant, and 1..3 strategy variants."
    $report.recommended_next_step = "Use TaskCount=1, MaxStepsPerVariant<=10, and no more than 3 strategies."
    Write-Reports -Report $report
    exit 0
}

$planJson = Join-Path $RepoRoot "runs\adapter_strategy_diagnostic\plan_report.json"
$planMd = Join-Path $RepoRoot "runs\adapter_strategy_diagnostic\plan_report.md"
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $planJson) | Out-Null
$savedGate = [Environment]::GetEnvironmentVariable("ALLOW_ADAPTER_STRATEGY_DIAGNOSTIC")
Remove-Item Env:\ALLOW_ADAPTER_STRATEGY_DIAGNOSTIC -ErrorAction SilentlyContinue
try {
    & powershell -ExecutionPolicy Bypass -File (Join-Path $RepoRoot "scripts\82_plan_adapter_strategy_action_scale_diagnostics.ps1") `
        -JsonReportPath $planJson `
        -MarkdownReportPath $planMd | Out-Null
} finally {
    if ($null -ne $savedGate) { $env:ALLOW_ADAPTER_STRATEGY_DIAGNOSTIC = $savedGate }
}
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $planJson)) {
    $report.reason = "adapter-strategy diagnostics planner failed"
    $report.recommended_next_step = "Fix the planner before adapter-strategy diagnostics."
    Write-Reports -Report $report
    exit 0
}
...
$report.plan = $plan
if (-not $plan.ready_for_adapter_strategy_diagnostic_runner_implementation) {
    $report.reason = "planner did not authorize adapter-strategy diagnostic runner"
...
if ($null -eq (Get-Command wsl -ErrorAction SilentlyContinue)) {
    ...
    $report.recommended_next_step = "Configure WSL before adapter-strategy diagnostics."
    Write-Reports -Report $report
    exit 0
}
...
if (-not ($repoWsl.ok -and $smolvlaWsl.ok -and $checkpointRootWsl.ok -and $hfHomeWsl.ok -and $liberoRootWsl.ok -and $robosuiteRootWsl.ok -and $liberoDataRootWsl.ok -and $pythonProbe.ok)) {
    ...
    $report.recommended_next_step = "Fix WSL path/Python readiness before adapter-strategy diagnostics."
    Write-Reports -Report $report
    exit 0
}
...
foreach ($strategy in $Strategies) {
    $safeStrategy = ConvertTo-SafeName -Value $strategy
    $innerReportWin = Join-Path $RepoRoot "runs\adapter_strategy_diagnostic\$safeStrategy.json"
    $stdoutLogWin = Join-Path $RepoRoot "runs\adapter_strategy_diagnostic\$safeStrategy.stdout.log"
    ...
    if (-not ($innerReportWsl.ok -and $stdoutLogWsl.ok)) {
        $variantReports += [ordered]@{ strategy = $strategy; passed = $false; reason = "failed to map WSL report/log paths" }
        continue
    }
    $bashCommand = @"
...
"@
    ...
    $variant = [ordered]@{
        ...
        last_gripper_component = $null
        action_adapter_strategy_observed = $null
        error = $null
    }
    if (Test-Path -LiteralPath $innerReportWin) {
        try {
            $inner = Read-JsonFile -Path $innerReportWin
            $variant.inner_report = $inner
            ...
            if ($tasks.Count -gt 0) {
                ...
                if ($lastTask.last_env_action_preview.Count -gt 0) {
                    $variant.last_gripper_component = [double]$lastTask.last_env_action_preview[-1]
                }
                $variant.action_adapter_strategy_observed = $lastTask.last_action_adapter_metadata.strategy
            }
...
if ($completed.Count -gt 0) {
    ...
    $report.result.best_strategy = $best.strategy
...
$report.adapter_strategy_diagnostic_passed = ($completed.Count -eq $Strategies.Count)
$report.decision = if ($report.adapter_strategy_diagnostic_passed) { "proceed" } else { "stop" }
$report.reason = if ($report.adapter_strategy_diagnostic_passed) {
    "bounded adapter-strategy diagnostic completed for all requested strategies"
} else {
    "one or more adapter-strategy diagnostic variants failed"
}
$report.recommended_next_step = if ($report.adapter_strategy_diagnostic_passed) {
    "Generate an adapter-strategy diagnostic summary and decide whether action-scale or prompt diagnostics are needed. Keep evidence diagnostic only."
} else {
    "Inspect failed adapter-strategy variants before additional diagnostics."
}

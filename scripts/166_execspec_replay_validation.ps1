param(
    ...
    [string]$JsonReportPath = "reports\execspec_state3_replay_validation.json",
    [string]$MarkdownReportPath = "reports\execspec_state3_replay_validation.md",
    [int]$MaxCalibrationDemos = 5,
    [int]$MaxEvalDemos = 3,
    [int]$MaxReplayEvalDemos = 3,
    [int]$MaxActionsPerDemo = 300,
    [int]$MaxStepsCap = 300,
    [int]$PostSignalMargin = 20,
    [int]$CameraSize = 64,
    [string]$ReplayMismatches = "gripper_sign_flip,translation_scale_mismatch,rotation_scale_mismatch,global_action_scale_mismatch,per_dimension_scale_mismatch,gripper_threshold_0_1_mismatch,range_clipping_mismatch",
    [string]$ReplayMethods = "wrong_executable_spec_replay,clipping_only,global_affine_calibration,diagonal_affine_calibration,gripper_only_calibration,full_execspec_repair",
    [string]$CalibrationSensitivitySizes = "1,3,5",
    [switch]$SkipDefaultResetSanity,
...
Write-Host "ExecSpec STATE 3 replay validation"
Write-Host "Repo root: $RepoRoot"
Write-Host "This script fits repair layers on calibration HDF5 demos and evaluates multiple held-out demos."
Write-Host "Exact-init replay runs only under ALLOW_EXECSPEC_STATE3_REPLAY_VALIDATION=1."
Write-Host "It does not download, install, load VLA models, infer learned policies, train neural models, use GPU, execute OpenVLA-OFT, run benchmark rollouts, or make paper claims."
...
if ($MaxEvalDemos -lt 1 -or $MaxEvalDemos -gt 10) {
    Write-Host "Refusing: MaxEvalDemos must be between 1 and 10."
    exit 14
}
if ($MaxReplayEvalDemos -lt 1 -or $MaxReplayEvalDemos -gt $MaxEvalDemos) {
    Write-Host "Refusing: MaxReplayEvalDemos must be between 1 and MaxEvalDemos."
    exit 15
}
...
$defaultResetArg = @()
if (-not $SkipDefaultResetSanity.IsPresent) {
    $defaultResetArg = @("--include-default-reset-sanity")
}

if ([Environment]::GetEnvironmentVariable("ALLOW_EXECSPEC_STATE3_REPLAY_VALIDATION") -ne "1") {
    & $Python -m tca_map.execspec.replay_validation `
        ...
        --max-replay-eval-demos $MaxReplayEvalDemos `
        ...
        --replay-methods $ReplayMethods `
        --calibration-sensitivity-sizes $CalibrationSensitivitySizes `
        @defaultResetArg
    exit $LASTEXITCODE
}
...
$defaultResetFlag = ""
if (-not $SkipDefaultResetSanity.IsPresent) {
    $defaultResetFlag = "--include-default-reset-sanity"
}

$bashCommand = @"
export PYTHONPATH='$($repoWsl.stdout)';
export MUJOCO_GL=osmesa;
export ALLOW_EXECSPEC_STATE3_REPLAY_VALIDATION=1;
$WslPython -m tca_map.execspec.replay_validation --manifest '$($manifestWsl.stdout)' --data-root '$($dataRootWsl.stdout)' --report-json '$($jsonReportWsl.stdout)' --report-md '$($markdownReportWsl.stdout)' --libero-root '$($liberoRootWsl.stdout)' --robosuite-root '$($robosuiteRootWsl.stdout)' --max-calibration-demos $MaxCalibrationDemos --max-eval-demos $MaxEvalDemos --max-replay-eval-demos $MaxReplayEvalDemos --max-actions-per-demo $MaxActionsPerDemo --max-steps-cap $MaxStepsCap --post-signal-margin $PostSignalMargin --camera-size $CameraSize --replay-mismatches '$ReplayMismatches' --replay-methods '$ReplayMethods' --calibration-sensitivity-sizes '$CalibrationSensitivitySizes' $defaultResetFlag
"@
$bashCommand = $bashCommand -replace "`r", ""
$result = Invoke-SafeCommand -Command @("wsl", "bash", "-lc", $bashCommand) -TimeoutSec $TimeoutSeconds
if (-not $result.ok) {
    Write-Host "ExecSpec STATE 3 replay validation command failed."

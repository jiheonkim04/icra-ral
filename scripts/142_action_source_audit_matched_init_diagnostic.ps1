param(
    ...
    [string]$JsonReportPath = "reports\action_source_audit_matched_init_diagnostic_report.json",
    [string]$MarkdownReportPath = "reports\action_source_audit_matched_init_diagnostic_report.md",
    [int]$MaxStepsCap = 300,
...
Write-Host "Action-source audit and matched-init diagnostic"
Write-Host "Repo root: $RepoRoot"
Write-Host "This script audits action provenance and runs only bounded matched-init candidate replay after a green readiness gate. It does not train, run GPU jobs, load VLA models, download assets, execute OpenVLA-OFT, run benchmark rollouts, or make paper-grade claims."
...
if ($MaxStepsCap -lt 1 -or $MaxStepsCap -gt 300) {
    Write-Host "Refusing: MaxStepsCap must be between 1 and 300."
    exit 12
}
...
if ([Environment]::GetEnvironmentVariable("ALLOW_ACTION_SOURCE_AUDIT_ROLLOUT") -ne "1") {
    & $Python -m tca_map.datasets.libero_action_source_audit_matched_init_diagnostic `
    ...
    if (Test-Path -LiteralPath $jsonFullPath) {
        try {
            ...
            if ([bool]$existingReport.result.passed) {
                Write-Host "Action-source audit report passed even though the WSL command did not exit cleanly."
                Get-Content -LiteralPath $jsonFullPath -Raw -Encoding UTF8
                exit 0
            }
        } catch {}
    }
    Write-Host "Action-source audit matched-init diagnostic command failed."

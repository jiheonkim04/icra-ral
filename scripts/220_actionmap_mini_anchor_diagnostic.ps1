param(
    ...
    [int]$TransBins = 7,
    [int]$RotBins = 7,
    [string]$JsonReportPath = "reports\actionmap_mini_anchor_state1_result.json",
    [string]$MarkdownReportPath = "reports\actionmap_mini_anchor_state1_result.md"
)
...
Write-Host "ActionMap mini-anchor gate diagnostic"
Write-Host "Repo root: $RepoRoot"
Write-Host "This script trains tiny CPU NumPy action heads on local LIBERO HDF5 action labels. It does not attempt full official ActionMap reproduction, implement Target-Grounded ActionMap, download, use GPU, run rollouts, import VLA models or simulators, access tokens, execute OpenVLA-OFT, or make paper claims."
...
if ($setGates.Count -gt 0) {
    Write-Host ("Refusing ActionMap mini-anchor diagnostic while dangerous gates are set: " + ($setGates -join ", "))
    exit 20
}

if ([Environment]::GetEnvironmentVariable("ALLOW_TINY_TRAINING") -ne "1") {
    Write-Host "ALLOW_TINY_TRAINING=1 is required for this bounded tiny CPU diagnostic."
    exit 21
}

"""Run RAR-VLA development-only Stage 0 audit."""
...
from tca_map.smolvla.rar_vla import PROPOSAL_HASH, audit_rar_records  # noqa: E402
...
def _write_md(path: Path, report: Mapping[str, Any]) -> None:
    ...
    history = report.get("history_feature_manifest") or {}
    observability = report.get("residual_observability_summary") or {}
    discontinuity = report.get("discontinuity_diagnostics") or {}
    ...
    lines = [
        ...
        f"- future actions used at inference: `{source_gate.get('future_actions_used_at_inference')}`",
        f"- CALA latents used at inference: `{source_gate.get('cala_latents_used_at_inference')}`",
        f"- residual predictability margin: `{observability.get('residual_predictability_margin')}`",
        f"- best trivial baseline: `{observability.get('best_trivial_baseline')}`",
        f"- residual headroom L2 validation: `{report['residual_headroom_l2_validation']}`",
        f"- initial action delta p95: `{report['initial_action_delta_p95']}`",
        f"- base action validity: `{report['base_action_validity']}`",
        f"- residual gradient norm: `{gradient.get('residual_head_gradient_norm')}`",
        f"- gate surrogate gradient norm: `{gradient.get('gate_surrogate_gradient_norm')}`",
        "",
        ...
        "",
        "```json",
        json.dumps(discontinuity, indent=2, sort_keys=True),
        "```",
        "",
        "Gradient audit:",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--json-output", default="reports/rar_vla/development_audit.json")
    parser.add_argument("--md-output", default="reports/rar_vla/development_audit.md")
    parser.add_argument("--source-gate-output", default="reports/rar_vla/source_gate_manifest.json")
    parser.add_argument("--history-output", default="reports/rar_vla/history_feature_manifest.json")
    parser.add_argument("--split-output", default="reports/rar_vla/split_manifest.json")
    args = parser.parse_args()

    artifact = _read_json(Path(args.prediction_artifact))
    report = audit_rar_records(
        artifact["records"],
    ...
    _write_json(Path(args.history_output), report["history_feature_manifest"])

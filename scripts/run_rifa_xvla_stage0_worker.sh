...
CONTRACT="configs/rifa_xvla_stage0_frozen_contract.json"

mkdir -p "$RUN_DIR"
printf '%s\n' "$$" > "$RUN_DIR/wsl_shell_pid.txt"
printf '%s\n' "$(date --iso-8601=seconds)" > "$RUN_DIR/worker_started_at.txt"
printf '%s\n' "HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 $PYTHON_BIN scripts/run_rifa_xvla_stage0.py --run-dir $RUN_DIR --contract $CONTRACT" > "$RUN_DIR/exact_command.txt"

HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 TOKENIZERS_PARALLELISM=false MUJOCO_GL=egl \
  "$PYTHON_BIN" scripts/run_rifa_xvla_stage0.py --run-dir "$RUN_DIR" --contract "$CONTRACT"
CODE=$?

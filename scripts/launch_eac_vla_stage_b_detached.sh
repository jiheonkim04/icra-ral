...
RUN_ROOT="${RUN_ROOT:-runs/eac_vla_stage_b}"
PARTIAL_RESULT="reports/eac_vla/stage_b_partial_result.json"
FINAL_RESULT="reports/eac_vla/stage_b_result.json"
MANIFEST="reports/eac_vla/stage_b_manifest.json"
RUNNER_VALIDATION="reports/eac_vla/stage_a_runner_validation.json"
REPORT_STATUS="reports/eac_vla/stage_b_status.json"
...
resume_command="cd ${REPO_ROOT} && ${PYTHON_BIN} scripts/run_eac_vla_stage_a.py --mode stage-b"
printf '%s\n' "$resume_command" > "${run_dir}/resume_command.txt"

partial_count() {
  "$PYTHON_BIN" -c 'import json, pathlib
p = pathlib.Path("reports/eac_vla/stage_b_partial_result.json")
...
"$python_bin" scripts/run_eac_vla_stage_a.py --mode stage-b --stage-b-status-output reports/eac_vla/stage_b_status.json &
child_pid=$!
...
exit "$code"
' eac_stage_b "$REPO_ROOT" "$PYTHON_BIN" "$run_dir" > "${run_dir}/stdout.log" 2> "${run_dir}/stderr.log" < /dev/null &

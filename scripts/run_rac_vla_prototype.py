"""RAC-VLA closed-loop prototype runner."""
...
from tca_map.smolvla.rac_vla import (  # noqa: E402
    PROPOSAL_HASH,
    TASK_KEYS,
    VARIANTS,
...
RAC_RESET_IDENTITY_BASE = 20261101
MAX_OFFICIAL_INITIAL_STATE_COUNT = 50
HIDDEN_SHIFT_LABEL = 1
TASKS = [
...
STAGE_A_IDENTITIES = list(range(20261101, 20261106))
STAGE_B_IDENTITIES = list(range(20261106, 20261126))
STAGE_B_EXPANSION_IDENTITIES = list(range(20261126, 20261146))
...
def _write_md(path: Path, title: str, report: Mapping[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- hidden shift: `{report.get('hidden_shift_name')}`",
        "",
...
def _identity_to_initial_state_index(identity: int) -> int:
    index = int(identity) - RAC_RESET_IDENTITY_BASE
...
def _run_episode(
    *,
    ...
    success = False
    previous_command = np.zeros(7, dtype=np.float64)
    try:
        ...
        for step in range(_max_steps(env, max_eval_steps)):
            ...
            if pending is not None:
                history.append({**pending, "delta_state": (state - np.asarray(pending["state"], dtype=np.float64)).tolist()})
            ...
            command, diagnostics = apply_rac_action(
                runtime,
                variant=str(row["variant"]),
                state=state,
                action=base_action,
                previous_action=previous_command,
                chunk_index_fraction=rho,
                task_key=str(row["task_key"]),
                history=history,
            )
            executed = apply_environment_shift(command, HIDDEN_SHIFT_LABEL)
            gates.append(float(diagnostics.get("gate", 0.0) or 0.0))
            action_deltas.append(float(diagnostics.get("action_delta_l2", 0.0) or 0.0))
            confidences.append(float(diagnostics.get("confidence", 0.0) or 0.0))
            predicted_labels.append(int(diagnostics.get("predicted_label", 0) or 0))
            ...
            pending = {
                ...
                "step": int(step),
                "state": state.tolist(),
                "action": np.asarray(command, dtype=np.float64).reshape(-1).tolist(),
                "previous_action": previous_command.tolist(),
                "chunk_index_fraction": float(rho),
            }
            previous_command = np.asarray(command, dtype=np.float64).reshape(-1)
            if done:
                break
        steps = int(step + 1 if "step" in locals() else 0)
        heavy_calls = int(np.ceil(steps / max(1, horizon)))
        nonzero_predictions = [label for label in predicted_labels if int(label) != 0]
        return {
            **dict(row),
            ...
            "mean_confidence": _round(float(np.mean(confidences)) if confidences else 0.0, 6),
            "nonzero_prediction_rate": _round(len(nonzero_predictions) / max(1, len(predicted_labels)), 6),
...
def _summarize(rows: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        by_variant[variant] = {
            ...
            "mean_confidence": _round(float(np.mean([float(row.get("mean_confidence", 0.0) or 0.0) for row in valid_rows])) if valid_rows else 0.0, 6),
            "mean_nonzero_prediction_rate": _round(float(np.mean([float(row.get("nonzero_prediction_rate", 0.0) or 0.0) for row in valid_rows])) if valid_rows else 0.0, 6),
    ...
    strongest = max((name for name in VARIANTS if name != "rac_full"), key=lambda name: by_variant[name]["task_balanced_success_rate"])
    return {
...
def _paired_bootstrap_ci(deltas: list[float], *, seed: int = 2026071402, samples: int = 5000) -> list[float]:
    if not deltas:
        return [0.0, 0.0]
...
def _paired_vs_full(rows: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for variant in VARIANTS:
        ...
        wins = losses = ties = 0
        for row in rows:
            if row.get("variant") != "rac_full" or row.get("exception"):
                continue
...
def _stage_a_decision(summary: Mapping[str, Any]) -> str:
    ...
    ablation = by["rac_no_consequence_ablation"]
    prior = by["reflective_history_proxy"]
    simple = by["online_diagonal_inverse_gain"]
    if float(full.get("mean_gate_activation_rate", 0.0) or 0.0) <= 0.0:
        return "STAGE_A_KILL_NO_MECHANISM_ACTIVATION"
    if full["successes"] == 0 and max(strongest["successes"], ablation["successes"], prior["successes"], simple["successes"]) >= 4:
    ...
    if float(simple["task_balanced_success_rate"]) - float(full["task_balanced_success_rate"]) >= 0.30:
        return "STAGE_A_KILL_CLEAR_SIMPLE_BASELINE_DOMINANCE"
...
def _stage_b_decision(summary: Mapping[str, Any], paired: Mapping[str, Any]) -> str:
    ...
    by = summary["by_variant"]
    full_rate = float(by["rac_full"]["task_balanced_success_rate"])
    strongest_name = str(summary["strongest_baseline"])
    strongest_rate = float(by[strongest_name]["task_balanced_success_rate"])
    ablation_rate = float(by["rac_no_consequence_ablation"]["task_balanced_success_rate"])
    prior_rate = float(by["reflective_history_proxy"]["task_balanced_success_rate"])
    simple_rate = float(by["online_diagonal_inverse_gain"]["task_balanced_success_rate"])
    if float(by["rac_full"].get("mean_gate_activation_rate", 0.0) or 0.0) <= 0.0:
        return "STAGE_B_KILL_NO_MECHANISM_ACTIVATION"
    if full_rate > max(strongest_rate, ablation_rate, prior_rate, simple_rate) and full_rate - strongest_rate >= 0.10:
        return "STAGE_B_PROTOTYPE_GO"
    for explainer in ("reflective_history_proxy", "rac_no_consequence_ablation", "online_diagonal_inverse_gain"):
        if float(by[explainer]["task_balanced_success_rate"]) >= full_rate:
            return "STAGE_B_KILL_BASELINE_OR_ABLATION_EXPLAINS_RESULT"
    if full_rate < float(by["base_smolvla_shifted"]["task_balanced_success_rate"]):
...
def _load_runtime(args: argparse.Namespace) -> dict[str, Any]:
    validation = json.loads(Path(args.validation_search).read_text(encoding="utf-8-sig"))
    selected = dict(validation["selected_config"])
    records = _read_jsonl(Path(args.development_records))
    return fit_rac_runtime(records, selected)
...
def _stage_mode(args: argparse.Namespace, *, stage: str) -> dict[str, Any]:
    ...
    report = {
        ...
        "reset_identity_base": RAC_RESET_IDENTITY_BASE,
        "identities": identities,
        "hidden_shift_label": HIDDEN_SHIFT_LABEL,
        "hidden_shift_name": "x_attenuate",
        "validation_search_path": str(args.validation_search),
        "validation_search_sha256": _sha256_file(Path(args.validation_search)),
        "selected_config": dict(runtime.get("selected_config") or {}),
        "planned_episode_count": len(rows),
        "completed_episode_count": len(episodes),
        "episodes": episodes,
        "summary": summary,
        "paired_vs_rac_full": paired,
    ...
    _write_md(md_path, f"RAC-VLA {stage.upper()} Result", report)
    return report
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--validation-search", default="reports/rac_vla/validation_search.json")
    parser.add_argument("--stage-a-output", default="reports/rac_vla/stage_a_result.json")
    parser.add_argument("--stage-a-md", default="reports/rac_vla/stage_a_result.md")
    parser.add_argument("--stage-a-partial-output", default="reports/rac_vla/stage_a_partial_result.json")
    parser.add_argument("--stage-b-output", default="reports/rac_vla/stage_b_result.json")
    parser.add_argument("--stage-b-md", default="reports/rac_vla/stage_b_result.md")
    parser.add_argument("--stage-b-partial-output", default="reports/rac_vla/stage_b_partial_result.json")
    parser.add_argument("--stage-b-expansion-output", default="reports/rac_vla/stage_b_expansion_result.json")
    parser.add_argument("--stage-b-expansion-md", default="reports/rac_vla/stage_b_expansion_result.md")
    parser.add_argument("--stage-b-expansion-partial-output", default="reports/rac_vla/stage_b_expansion_partial_result.json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if set(TASK_KEYS) != {f"{task['suite']}/task_{task['task_id']}" for task in TASKS}:
        raise SystemExit("RAC task constants disagree with runner task manifest")
    report = run(args)

...
- bounded rollout: only after simulator import/render smoke passes, task count <=5, runtime <=30 minutes, no OpenVLA-OFT, no external service/token, no unbounded render loop, no paper claim. This covers toy MuJoCo or LIBERO/RoboSuite zero-action diagnostic rollouts only; benchmark rollout, learned-policy rollout, multi-seed rollout, and paper-grade claims remain separate stop gates.

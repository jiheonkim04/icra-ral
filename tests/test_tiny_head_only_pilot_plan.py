...
def test_tiny_head_only_pilot_planner_is_planning_only(tmp_path):
    ...
    assert report["ready_for_tiny_training_risk_assessment"] is True
    assert report["policy"]["planning_only"] is True
    assert report["policy"]["risk_assessed_autonomy_for_tiny_training_smoke"] is True
    assert report["policy"]["risk_assessment_required_before_training"] is True

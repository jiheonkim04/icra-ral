...
def test_active_state_records_amp_selection_and_rap_stage_0_failure() -> None:
    ...
    assert state["preregistration"] == "reports/mci_vla/preregistration.md"
    assert state["preregistration_decision"] == "MCI_PREREGISTRATION_FROZEN_PROTOTYPE_PROTOCOL_PENDING"
    ...
    assert cycle35["candidate_ids"] == ["MHS-VLA", "VQRM-VLA", "IBER-VLA"]
    ...
    assert mhs["reviewer_attack_pending"] is False
    ...
    assert mhs["stage_0_implementation_pending"] is False
    assert mhs["stage_0_implementation_validated"] is True
    assert mhs["stage_0_pending"] is False
    assert mhs["serializer_preflight"] == "reports/mhs_vla/stage_0_serializer_preflight.json"
    ...
    assert impl35["serializer_preflight"] == "reports/mhs_vla/stage_0_serializer_preflight.json"
    assert impl35["py_compile_passed"] is True
    assert impl35["serializer_preflight_passed"] is True
    assert impl35["focused_mhs_tests_passed"] == 6
    ...
    assert selection36["preregistration_pending"] is False
    assert selection36["preregistration_frozen"] is True
    ...
    assert selection36["stage_0_pending"] is False
    assert selection36["stage_0_completed"] is True
    assert selection36["stage_0_decision"] == "DCCG_STAGE_0_DATA_FAILURE"
    ...
    assert proposal36["preregistration_pending"] is False
    assert proposal36["preregistration_frozen"] is True
    ...
    assert proposal36["stage_0_pending"] is False
    assert proposal36["stage_0_completed"] is True
    assert proposal36["stage_0_decision"] == "DCCG_STAGE_0_DATA_FAILURE"
    ...
    assert review36["preregistration_pending"] is False
    assert review36["preregistration_frozen"] is True
    ...
    assert review36["stage_0_pending"] is False
    assert review36["stage_0_completed"] is True
    assert review36["stage_0_decision"] == "DCCG_STAGE_0_DATA_FAILURE"
    ...
    assert rebuttal36["preregistration_pending"] is False
    assert rebuttal36["preregistration_frozen"] is True
    ...
    assert rebuttal36["stage_0_pending"] is False
    assert rebuttal36["stage_0_completed"] is True
    assert rebuttal36["stage_0_decision"] == "DCCG_STAGE_0_DATA_FAILURE"
    ...
    assert math36["hard_counts_are_diagnostics_not_unverified_gradients"] is True
    ...
    assert math36["gamma_values"] == [0.05, 0.1, 0.2]
    assert math36["gate_quantiles"] == [0.9, 0.95]
    assert math36["preregistration_pending"] is False
    ...
    assert math36["stage_0_pending"] is False
    assert math36["stage_0_completed"] is True
    assert math36["stage_0_decision"] == "DCCG_STAGE_0_DATA_FAILURE"
    ...
    assert prereg36["preregistration"] == "reports/dccg_vla/preregistration.md"
    assert prereg36["final_decision"] == "DCCG_PREREGISTRATION_FROZEN_PROTOTYPE_PROTOCOL_PENDING"
    assert prereg36["bounded_validation_search_max_configs"] == 6
    assert prereg36["prototype_protocol_pending"] is False
    assert prereg36["prototype_protocol_frozen"] is True
    protocol36 = state["epoch_4_cycle_36_dccg_prototype_protocol"]
    assert protocol36["method"] == "DCCG-VLA"
    assert protocol36["proposal_hash"] == DCCG_PROPOSAL_HASH
    assert protocol36["prototype_protocol"] == "reports/dccg_vla/prototype_protocol.md"
    assert protocol36["final_decision"] == "DCCG_PROTOTYPE_PROTOCOL_FROZEN_STAGE_0_IMPLEMENTATION_PENDING"
    assert protocol36["required_helper"] == "tca_map/smolvla/dccg_vla.py"
    assert protocol36["required_runner"] == "scripts/run_dccg_vla_stage0.py"
    assert protocol36["required_tests"] == "tests/test_dccg_vla.py"
    ...
    assert protocol36["stage_0_pending"] is False
    assert protocol36["stage_0_completed"] is True
    assert protocol36["stage_0_decision"] == "DCCG_STAGE_0_DATA_FAILURE"
    ...
    outcome36 = state["epoch_4_cycle_36_dccg_stage_0_outcome"]
    assert outcome36["final_decision"] == "DCCG_STAGE_0_DATA_FAILURE"
    assert outcome36["failure_class"] == "DATA_FAILURE"
    assert outcome36["valid_scientific_result"] is False
    assert outcome36["stage_0_is_closed_loop_scientific_kill"] is False
    assert outcome36["stage_0_result"] == "reports/dccg_vla/stage_0_result.json"
    assert outcome36["stage_0_adjudication"] == "reports/dccg_vla/stage_0_adjudication.md"
    assert outcome36["completed_model_row_count"] == 0
    assert outcome36["planned_model_row_count"] == 0
    assert outcome36["exception_count"] == 0
    assert outcome36["duplicate_manifest_key_count"] == 0
    assert outcome36["duplicate_partial_key_count"] == 0
    assert outcome36["missing_manifest_key_count"] == 0
    assert outcome36["extra_partial_key_count"] == 0
    assert outcome36["split_overlap_key_count"] == 0
    assert outcome36["key_sets_equal"] is True
    assert outcome36["matching_frozen_dccg_rows"] == 0
    assert outcome36["dccg_rescue_allowed"] is False
    cycle37 = state["epoch_4_cycle_37_candidate_search"]
    assert cycle37["candidate_search_pending"] is False
    assert cycle37["candidate_count_required"] == 3
    assert cycle37["candidate_count_generated"] == 3
    assert cycle37["candidate_ids"] == ["CSPR-VLA", "PGF-VLA", "POTR-VLA"]
    assert cycle37["selected_method"] == "CSPR-VLA"
    assert cycle37["selected_score"] == 90
    assert cycle37["selection_decision"] == "CSPR_PROPOSAL_FROZEN_REVIEWER_ATTACK_PENDING"
    assert cycle37["closest_prior"] == "DySL-VLA"
    assert cycle37["available_cached_smolvla_base_rows"] == 640
    assert cycle37["cache_covered_tasks"] == [
        "libero_10/task_5",
        "libero_goal/task_5",
        "libero_object/task_3",
        "libero_spatial/task_3",
    ]
    assert cycle37["cache_covered_demo_ids"] == list(range(10))
    assert cycle37["policy_order"] == [
        "smolvla_base",
    ...
    assert cycle37["first_serious_comparison_includes_closest_prior"] is True
    assert cycle37["previous_decision"] == "DCCG_STAGE_0_DATA_FAILURE"
    assert cycle37["dccg_rescue_allowed"] is False
    selection37 = state["epoch_4_cycle_37_candidate_selection"]
    assert selection37["method"] == "CSPR-VLA"
    assert selection37["closest_prior"] == "DySL-VLA"
    assert selection37["candidate_count"] == 3
    assert selection37["selected_score"] == 90
    assert selection37["researcher_proposal_pending"] is False
    assert selection37["researcher_proposal_frozen"] is True
    assert selection37["proposal_hash"] == CSPR_PROPOSAL_HASH
    assert selection37["proposal_hash_file"] == "reports/cspr_vla/proposal_hash.txt"
    ...
    assert selection37["training_happened"] is False
    assert selection37["validation_search_happened"] is False
    assert selection37["closed_loop_experiment_happened"] is False
    assert selection37["confirmatory_test_tuning_happened"] is False
    proposal37 = state["epoch_4_cycle_37_cspr_researcher_proposal"]
    assert proposal37["method"] == "CSPR-VLA"
    assert proposal37["final_decision"] == "CSPR_PROPOSAL_FROZEN_REVIEWER_ATTACK_PENDING"
    assert proposal37["proposal_hash"] == CSPR_PROPOSAL_HASH
    assert proposal37["researcher_proposal"] == "reports/cspr_vla/researcher_proposal.md"
    assert proposal37["researcher_proposal_frozen"] is True
    ...
    assert proposal37["closed_loop_experiment_happened"] is False
    assert proposal37["confirmatory_test_tuning_happened"] is False
    review37 = state["epoch_4_cycle_37_cspr_reviewer_attack"]
    assert review37["method"] == "CSPR-VLA"
    assert review37["proposal_hash"] == CSPR_PROPOSAL_HASH
    assert review37["reviewer_attack"] == "reports/cspr_vla/reviewer_attack.md"
    assert review37["final_decision"] == "REVIEWER_ATTACK_CONDITIONAL_PASS_REBUTTAL_REQUIRED"
    assert len(review37["required_conditions"]) == 10
    assert review37["dySL_proxy_must_be_transparent"] is True
    assert review37["criticality_label_health_required"] is True
    ...
    assert cycle38["candidate_search_pending"] is False
    assert cycle38["candidate_count_required"] == 3
    assert cycle38["candidate_count_generated"] == 3
    assert cycle38["candidate_ids"] == ["MCI-VLA", "ICR-VLA", "OAB-VLA"]
    assert cycle38["selected_method"] == "MCI-VLA"
    assert cycle38["selected_score"] == 92
    assert cycle38["selection_decision"] == "MCI_CANDIDATE_SELECTED_RESEARCHER_PROPOSAL_PENDING"
    ...
    assert cycle38["closest_prior"] == "RoVLA"
    assert cycle38["policy_order"] == [
        "smolvla_base",
        "rovla_multiconsistency_proxy",
        "mci_full",
        "mci_no_consistency_code_ablation",
    ...
    assert cycle38["first_serious_comparison_includes_closest_prior"] is True
    selection38 = state["epoch_4_cycle_38_candidate_selection"]
    assert selection38["method"] == "MCI-VLA"
    assert selection38["closest_prior"] == "RoVLA"
    ...
    assert selection38["preregistration_pending"] is False
    assert selection38["preregistration_frozen"] is True
    assert selection38["preregistration"] == "reports/mci_vla/preregistration.md"
    assert selection38["preregistration_decision"] == "MCI_PREREGISTRATION_FROZEN_PROTOTYPE_PROTOCOL_PENDING"
    assert selection38["prototype_protocol_pending"] is False
    ...
    assert proposal38["preregistration_pending"] is False
    assert proposal38["preregistration_frozen"] is True
    assert proposal38["preregistration"] == "reports/mci_vla/preregistration.md"
    assert proposal38["preregistration_decision"] == "MCI_PREREGISTRATION_FROZEN_PROTOTYPE_PROTOCOL_PENDING"
    assert proposal38["prototype_protocol_pending"] is False
    ...
    assert review38["preregistration_pending"] is False
    assert review38["preregistration_frozen"] is True
    assert review38["preregistration"] == "reports/mci_vla/preregistration.md"
    assert review38["preregistration_decision"] == "MCI_PREREGISTRATION_FROZEN_PROTOTYPE_PROTOCOL_PENDING"
    assert review38["prototype_protocol_pending"] is False
    ...
    assert rebuttal38["preregistration_pending"] is False
    assert rebuttal38["preregistration_frozen"] is True
    assert rebuttal38["preregistration"] == "reports/mci_vla/preregistration.md"
    assert rebuttal38["preregistration_decision"] == "MCI_PREREGISTRATION_FROZEN_PROTOTYPE_PROTOCOL_PENDING"
    assert rebuttal38["prototype_protocol_pending"] is False
    ...
    assert audit38["preregistration_pending"] is False
    assert audit38["preregistration_frozen"] is True
    assert audit38["preregistration"] == "reports/mci_vla/preregistration.md"
    assert audit38["preregistration_decision"] == "MCI_PREREGISTRATION_FROZEN_PROTOTYPE_PROTOCOL_PENDING"
    assert audit38["prototype_protocol_pending"] is False
    ...
    prereg38 = state["epoch_4_cycle_38_mci_preregistration"]
    assert prereg38["method"] == "MCI-VLA"
    assert prereg38["proposal_hash"] == MCI_PROPOSAL_HASH
    assert prereg38["preregistration"] == "reports/mci_vla/preregistration.md"
    assert prereg38["mathematical_audit"] == "reports/mci_vla/mathematical_mechanism_audit.md"
    assert prereg38["final_decision"] == "MCI_PREREGISTRATION_FROZEN_PROTOTYPE_PROTOCOL_PENDING"
    assert prereg38["discovery_demo_ids"] == "0..7"
    assert prereg38["validation_demo_ids"] == "8..9"
    assert prereg38["discovery_min_rows"] == 512
    assert prereg38["validation_min_rows"] == 128
    assert prereg38["bounded_validation_search_max_configs"] == 6
    assert len(prereg38["bounded_validation_search_configs"]) == 6
    assert len(prereg38["stage_0_stop_classes"]) == 5
    assert prereg38["deterministic_action_kl_allowed"] is False
    assert prereg38["first_serious_comparison_includes_closest_prior"] is True
    assert prereg38["prototype_protocol_pending"] is False
    ...
    assert prereg38["confirmatory_test_tuning_happened"] is False
    protocol38 = state["epoch_4_cycle_38_mci_prototype_protocol"]
    assert protocol38["method"] == "MCI-VLA"
    assert protocol38["proposal_hash"] == MCI_PROPOSAL_HASH
    assert protocol38["prototype_protocol"] == "reports/mci_vla/prototype_protocol.md"
    assert protocol38["preregistration"] == "reports/mci_vla/preregistration.md"
    assert protocol38["final_decision"] == "MCI_PROTOTYPE_PROTOCOL_FROZEN_STAGE_0_IMPLEMENTATION_PENDING"
    assert protocol38["helper"] == "tca_map/smolvla/mci_vla.py"
    assert protocol38["runner"] == "scripts/run_mci_vla_stage0.py"
    assert protocol38["focused_tests"] == "tests/test_mci_vla.py"
    assert protocol38["serializer_preflight"] == "reports/mci_vla/stage_0_serializer_preflight.json"
    assert protocol38["stage_0_result"] == "reports/mci_vla/stage_0_result.json"
    assert len(protocol38["stage_0_stop_classes"]) == 5
    assert protocol38["worker_safety_resume_required"] is True
    assert protocol38["duplicate_key_checks_required"] is True
    ...
    assert protocol38["confirmatory_test_tuning_happened"] is False
    ...
    assert brid_stage0["planned_model_row_count"] == 46080
    assert brid_stage0["exception_count"] == 0
    assert brid_stage0["stage_0_result"] == "reports/brid_vla/stage_0_result.json"
    assert brid_stage0["stage_0_partial"] == "reports/brid_vla/stage_0_partial.json"
    assert brid_stage0["stage_0_manifest"] == "reports/brid_vla/stage_0_manifest.json"
    assert "epoch_4_cycle_35_prior_mechanism_map_completed" in state["completed_stages"]
    ...
    assert "epoch_4_cycle_35_mhs_reviewer_attack_completed" in state["completed_stages"]
    ...
    assert "epoch_4_cycle_36_dccg_mathematical_audit_preregistered" in state["completed_stages"]
    assert "epoch_4_cycle_36_dccg_preregistration_pending" in state["completed_stages"]
    assert "epoch_4_cycle_36_dccg_preregistration_frozen" in state["completed_stages"]
    assert "epoch_4_cycle_36_dccg_prototype_protocol_pending" in state["completed_stages"]
    assert "epoch_4_cycle_36_dccg_prototype_protocol_frozen" in state["completed_stages"]
    assert "epoch_4_cycle_36_dccg_stage_0_implementation_pending" in state["completed_stages"]
    assert "epoch_4_cycle_37_candidate_search_pending" in state["completed_stages"]
    ...
    assert "epoch_4_cycle_37_cspr_stage_0_runner_implemented" in state["completed_stages"]
    assert "epoch_4_cycle_37_cspr_stage_0_implementation_validated" in state["completed_stages"]
    assert "epoch_4_cycle_37_cspr_stage_0_ready" in state["completed_stages"]
    ...
    assert "epoch_4_cycle_38_prior_mechanism_map_completed" in state["completed_stages"]
    ...
    assert "epoch_4_cycle_38_mci_preregistration_frozen" in state["completed_stages"]
    ...
    for stage in (
        ...
        "epoch_4_cycle_33_afid_prototype_protocol_frozen",
        "epoch_4_cycle_33_afid_stage_0_implementation_pending",
        "epoch_4_cycle_33_afid_stage_0_implementation_validated",
        "epoch_4_cycle_33_afid_stage_0_completed",
        "epoch_4_cycle_33_afid_stage_0_adjudicated",
        "epoch_4_cycle_33_afid_implementation_failure_recorded",
        "epoch_4_cycle_34_prior_mechanism_map_completed",
        "epoch_4_cycle_34_candidate_generation_completed",
        "epoch_4_cycle_34_brid_candidate_selected",

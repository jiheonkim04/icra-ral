...
def test_active_campaign_final_decision_is_nonterminal_pivot() -> None:
    ...
    assert "Current campaign decision: `EPOCH_3_CYCLE_1_CBFD_KILLED_PIVOT_REQUIRED`" in final
    assert "This is not a terminal decision." in final
    assert "READY_TO_DRAFT_RAL_PAPER_PACKAGE" in final
    assert "Begin Epoch 3 Cycle 2 candidate generation" in final
    assert "CBFD-VLA" in final

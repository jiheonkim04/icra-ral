...
def test_active_campaign_final_decision_is_nonterminal_pivot() -> None:
    ...
    assert "Current campaign decision: `EPOCH_4_CYCLE_2_CAVM_NON_GO_CONTINUE_CYCLE_3`" in final
    assert "This is not a terminal decision." in final
    assert "READY_TO_DRAFT_RAL_PAPER_PACKAGE" in final
    assert "begin Epoch 4 Cycle 3 candidate generation" in final
...
def test_active_campaign_state_records_governance_v2() -> None:
    ...
    assert state["next_action"].startswith("Begin Epoch 4 Cycle 3 candidate generation")

...
39. Bounded simulator render/reset-step risk planning. Done: `scripts\58_plan_simulator_render_reset.ps1` reports `ready_for_bounded_render_smoke_plan=true` after the passed import-only smoke.
40. Bounded simulator render-smoke branch. Implemented and attempted. Current local result: blocked because `MUJOCO_GL=osmesa` failed with `AttributeError: 'NoneType' object has no attribute 'glGetError'`.
41. Current gate: simulator offscreen graphics-stack readiness. Do not install system graphics packages, reset, step, rollout, or claim benchmark readiness unless the specific later risk assessment is green and inside budget.
...
5. Run a separate bounded render/reset-step risk planner. Done.
6. Create and run a separate bounded render-smoke branch only if the render-smoke assessment is green. Done; current local result is blocked by offscreen GL/OSMesa.
7. Resolve the offscreen graphics-stack blocker only through a separate risk assessment. Do not make system graphics changes automatically.
8. If render later passes, run a separate bounded reset/step-smoke risk assessment.
9. If reset/step passes, run a separate bounded tiny diagnostic rollout risk assessment.

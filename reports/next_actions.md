...
30. Current gate: no real LIBERO demonstration files or documented tiny subset exist under `LIBERO_DATA_ROOT`; full official dataset is about 100 GB and remains outside the 80 GB autonomous task budget.
31. Later: bounded simulator import-smoke only as a separate import-only task, tiny real/offline dataset interface smoke only if a real tiny subset exists, or larger compute handoff planning.
...
After the LoRA/QLoRA go/no-go update, tiny LoRA smoke runner, tiny LoRA comparison report, consolidated local pilot status report, risk-assessed policy update, LIBERO dataset risk planner, simulator readiness planner, 300-step budget alignment, bounded local pilot extension, bounded-extension status consolidation, official source resolution, source repo setup, and metadata-only subset construction, continue autonomously by choosing the next concrete task and running its risk assessment. The current concrete boundary is the dataset-file gate: run `scripts\48_plan_libero_offline_interface_smoke.ps1` to report stop/proceed for tiny local data files, then stop before actual real-data smoke until real demo files are present. The full official LIBERO dataset remains stopped because it is about 100 GB, above the 80 GB budget. Stop only if the assessment is ambiguous/outside budget or reaches external irreversible, OpenVLA-OFT, token/secret/payment/license, system-level, simulator execution, rollout, or paper-claim gates.

...
34. Current gate: tiny real/offline ActionMap vs TCA-Map comparison with `scripts\52_compare_libero_offline_actionmap_tca.ps1`.
35. After offline head comparison readiness: required tiny real/offline LoRA comparison and bounded local pilot report if each gate passes.
...
After the LoRA/QLoRA go/no-go update, tiny LoRA smoke runner, tiny LoRA comparison report, consolidated local pilot status report, risk-assessed policy update, LIBERO dataset risk planner, simulator readiness planner, 300-step budget alignment, bounded local pilot extension, bounded-extension status consolidation, official source resolution, source repo setup, metadata-only subset construction, official LIBERO data acquisition, h5py reader readiness, offline interface smoke gate, and summary consolidation, continue autonomously by choosing the next concrete task and running its risk assessment. The current concrete boundary is tiny real/offline counterfactual split construction from local LIBERO metadata/HDF5 inventory. Stop only if the split checker cannot find matched local demo pairs, the assessment is ambiguous/outside budget, or the task reaches external irreversible, OpenVLA-OFT, token/secret/payment/license, system-level, simulator execution, rollout, or paper-claim gates.

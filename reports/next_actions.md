...
43. Current gate: tiny diagnostic rollout risk assessment. Do not execute rollout while the current user instruction forbids rollout; prepare only a separate risk assessment/planner unless rollout becomes explicitly allowed.
...
9. Next: run a separate bounded tiny diagnostic rollout risk assessment only. Do not execute rollout while the current user instruction forbids rollout.

...
12. Run `scripts\66_plan_libero_policy_rollout_readiness.ps1` before any learned-policy rollout. Continue to a tiny learned-policy rollout runner only if that planner is green; otherwise reduce scope to WSL SmolVLA runtime readiness. Stop before multi-seed rollout, OpenVLA-OFT, full fine-tuning, external upload, or unsupported paper-level claims.

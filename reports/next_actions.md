...
28. Current gate: real LIBERO/LIBERO-CF-style data and simulator readiness are planning-only because local LIBERO/RoboSuite paths are missing and no official dataset source/size is recorded for autonomous acquisition.
29. Candidate next stages: bounded simulator import-smoke only if local WSL2/Linux paths are green, real dataset/tiny-subset setup only if local paths or an official source/size are ready, or larger compute handoff planning.

Current status: the bounded tiny head-only smoke, ActionMap vs TCA-Map head-only comparison report, tiny LoRA smoke runner, tiny LoRA comparison report, consolidated local pilot status, go/no-go summary, required LoRA adapter construction plan, LoRA tiny-smoke scaffold, TCA-Map + LoRA comparison plan, QLoRA feasibility check, LoRA/QLoRA go/no-go update, LIBERO dataset risk planner, simulator readiness planner, 300-step local pilot budget alignment, bounded cached-feature local pilot extension, and bounded-extension status consolidation have passed. LoRA/QLoRA are required experimental tracks after the head-only path, but not the main novelty. The repository now uses risk-assessed autonomous execution instead of broad approval-based hard-stops. Downloads, GPU tasks, bounded training, real dataset setup, simulator readiness, and bounded rollout should proceed automatically if a risk assessment is green and should stop only if risk is ambiguous, outside budget, external/irreversible, OpenVLA-OFT-related, or paper-claim-related.
...
After the LoRA/QLoRA go/no-go update, tiny LoRA smoke runner, tiny LoRA comparison report, consolidated local pilot status report, risk-assessed policy update, LIBERO dataset risk planner, simulator readiness planner, 300-step budget alignment, bounded local pilot extension, and bounded-extension status consolidation, continue autonomously by choosing the next concrete task and running its risk assessment. The current concrete boundary is real dataset/simulator readiness. It must remain planning-only until `LIBERO_ROOT`, `LIBERO_DATA_ROOT`, and `ROBOSUITE_ROOT` exist or an official dataset source plus expected size are documented. Stop only if the assessment is ambiguous/outside budget or reaches external irreversible, OpenVLA-OFT, token/secret/payment/license, system-level, simulator execution, rollout, or paper-claim gates.

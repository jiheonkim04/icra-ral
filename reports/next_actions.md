...
37. Current gate: WSL simulator dependency risk planning. `scripts\55_bounded_simulator_import_smoke.ps1` ran under task-local `ALLOW_SIMULATOR_IMPORT_SMOKE=1`; WSL path and `python3` probes passed, `libero` imported, and `robosuite` stopped on missing WSL `numpy`. Do not install dependencies, render, or rollout unless a separate risk assessment is green and inside budget.

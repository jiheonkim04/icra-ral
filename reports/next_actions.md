...
37. Current gate: simulator readiness/import-render risk assessment if installed locally. `scripts\43_plan_simulator_readiness.ps1` is planning-only, and its runtime report is now included in the local pilot and go/no-go summaries. Do not run simulator import, render smoke, or rollout unless a separate risk assessment is green and inside budget.

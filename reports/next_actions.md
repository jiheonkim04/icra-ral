...
31. Current gate: HDF5 reader dependency check with `scripts\50_check_libero_hdf5_reader.ps1`. If `h5py` is unavailable, run a separate dependency risk assessment before installing `h5py>=3.11`.
32. After HDF5 reader readiness: LIBERO data readiness check, tiny real/offline dataset interface smoke, counterfactual split construction, tiny ActionMap vs TCA-Map offline comparison, required tiny LoRA comparison, and bounded local pilot report if each gate passes.

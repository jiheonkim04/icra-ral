...
7D baseline reproduction update: final decision `READY_FOR_RA_L_METHOD_ON_SMOLVLA_7D`. Rank-8 `state_proj` LoRA plus the fixed LIBERO_7D adapter beat mean-action (`1.082453`) and the best ridge/MLP baseline (`0.518738`) with held-out action L2 `0.494959` on the same-task demo-holdout split. This is still local action-metric evidence, not rollout or paper evidence.

TG-7D Adapter update: final decision `KILL_CANONICALIZATION_DOMINATED`. The leakage-safe LIBERO-Para/fixed-7D split was feasible, but TG-7D held-out paraphrase L2 (`0.740922`) was beaten by canonicalization-only (`0.587661`), standard SmolVLA 7D LoRA/adapter (`0.600887`), and MLP (`0.619985`). The route is killed as formulated; the reusable artifacts are the split, target-prior audit, and bounded gate runner.

Post-canonicalization residual update: final decision `KILL_CANONICALIZATION_DOMINATED`. Residual mining found canonicalization clean-to-paraphrase delta `-0.000748`, no structured language/target residual, largest absolute residual in gripper error (`0.389255`), and no oracle/headroom evidence (`oracle_headroom_l2 = -0.137013`). Stop the local language/target route unless a new official benchmark slice exposes a large non-canonicalization residual.

...
`CSPR_PROTOTYPE_PROTOCOL_FROZEN_STAGE_0_IMPLEMENTATION_PENDING`. Stage 0 implementation validation is complete:
`tca_map/smolvla/cspr_vla.py`, `scripts/run_cspr_vla_stage0.py`, and
`tests/test_cspr_vla.py` are implemented; serializer preflight passed in
...
Selected method: `BRID-VLA`, Base-Residual Implicit Diffusion for SmolVLA
action chunks, score `94 / 100`.

Closest prior: Diffusion Policy (`https://diffusion-policy.cs.columbia.edu/`
and `https://github.com/real-stanford/diffusion_policy`), with positive
evidence for conditional denoising over action sequences across 12 tasks and
four manipulation benchmarks.
...
URF's single mechanism is a heteroscedastic residual-flow field around a frozen
SmolVLA Base action chunk. It routes bounded residual transport only where
predicted residual uncertainty supports intervention and otherwise preserves
Base. LoRA may only be implementation infrastructure; the scientific method is
uncertainty-routed residual transport.

The first serious comparison is frozen at the design level to

...
Repeated-residual screen `20260729..35`: natural-reset mining ended at `ddbe62c` with `NATURAL_RESET_SEARCH_SATURATED`; no more identity sweeps. SGL closed; OCR legal trace pass failed and is archived. Wrist-dropout task5 condition verified `2/2` clean to `0/2` dropout; AWF-XVLA Stage 0 no-go and reclassified as `SIMPLE_CAMERA_IMPUTATION_CONTROL`, not learned method kill. New steer requires condition generality before another method: preregistered frozen X-VLA-only 3-task × 3-identity clean/dropout paired study in `reports/wrist_dropout_generality_preregistration_result.json`; no Ours/training/task selection after outcomes; next run exactly 18 episodes and return one of the four wrist-dropout decisions.

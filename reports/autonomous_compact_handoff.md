...
Repeated-residual screen `20260729..35`: natural-reset mining ended at `ddbe62c` with `NATURAL_RESET_SEARCH_SATURATED`; no more identity sweeps. SGL closed; OCR legal trace pass failed and is archived. AWF-XVLA Stage 0 no-go and reclassified as `SIMPLE_CAMERA_IMPUTATION_CONTROL`. Wrist-dropout generality confirmed with frozen X-VLA only: clean `9/9`, dropout `0/9`, flips `9`, decision `WRIST_DROPOUT_REPEATED_PROBLEM_CONFIRMED`. RL4IL selected as closest external prior; official release has constant-label supervision/no checkpoints, so rollout not armed as-is. Action-sequence-oracle port preflight passed and prereg froze live reset-state queries (not same-index HDF5 demos), mask_1, and 9+9 Stage A; next implement/launch prior runner before Ours. See `reports/rl4il_action_oracle_port_preregistration_result.json`.

...
Repeated-residual screen `20260729..35`: natural-reset mining ended at `ddbe62c` with `NATURAL_RESET_SEARCH_SATURATED`; no more identity sweeps. SGL closed; OCR only allowed legal trace pass failed (`OCR_TRIGGER_OBSERVABILITY_FAIL`, `reports/ocr_xvla_trace_observability_result.json`) and is archived. Claim-specific condition verified: wrist-camera dropout partial observation on `libero_spatial/task5` clean identities `20260731/32` dropped X-VLA from clean `2/2` to dropout `0/2`, `reports/wrist_dropout_condition_verification_result.json`. One method selected/preregistered: AWF-XVLA, no-training agentview-to-wrist fill; Stage 0 result `AWF_XVLA_STAGE0_NO_GO`, still `0/2`, `reports/awf_xvla_stage0_result.json`; archive AWF and do not tune it on discovery failures.

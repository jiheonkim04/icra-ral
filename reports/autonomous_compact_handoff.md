...
Repeated-residual screen `20260729..35`: natural-reset mining ended at `ddbe62c` with `NATURAL_RESET_SEARCH_SATURATED`; no more identity sweeps. Closed task5 family remains Base/X-VLA/OpenVLA failures on `20260727/30/33` with clean X-VLA retention `20260731/32`; SGL stays closed by simple-control equivalence. OCR got its only allowed five-episode frozen-prior trace pass (`20260727/30/31/32/33`, no held-out, no Ours/training/checkpoint, legal trace fields only); result `OCR_TRIGGER_OBSERVABILITY_FAIL` in `reports/ocr_xvla_trace_observability_result.json` because legal RGB/proprio separation did not exceed the action-history-only baseline. OCR is archived; next select one claim-specific controlled condition or another official-prior ecosystem, then freeze candidate within the steer budget.

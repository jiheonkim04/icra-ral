...
Repeated-residual screen `20260729..35`: natural-reset mining ended at `ddbe62c` with `NATURAL_RESET_SEARCH_SATURATED`; no more identity sweeps. SGL/OCR closed; AWF is `SIMPLE_CAMERA_IMPUTATION_CONTROL`. Wrist-dropout generality confirmed: clean `9/9`, dropout `0/9`, flips `9`, decision `WRIST_DROPOUT_REPEATED_PROBLEM_CONFIRMED`. RL4IL official release has constant labels/no checkpoints, so local port is `MECHANISM_FAITHFUL_RL4IL_LOCAL_PORT`. PRIOR_MODULE_TRAINING passed: params `13512777`, steps `54`, gradients/weight changes/checkpoint reload true; one launcher repair consumed. PRIOR_CLOSED_LOOP_ROLLOUT passed on frozen 9+9 panel: clean `4/9`, mask_1 `3/9`, forwards `99`, decision `RL4IL_ACTION_ORACLE_PRIOR_LOCAL_RESIDUAL_ESTABLISHED`. Next: generate at most two Ours candidates around residual before any Ours training. See `reports/rl4il_action_oracle_prior_closed_loop_rollout_result.json`.

...
Repeated-residual screen `20260729..35`: natural-reset mining ended at `ddbe62c` with `NATURAL_RESET_SEARCH_SATURATED`; no more identity sweeps. Closed task5 family remains Base/X-VLA/OpenVLA failures on `20260727/30/33` with clean X-VLA retention `20260731/32`; SGL stays closed by simple-control equivalence. OCR got its only allowed five-episode legal trace pass and failed (`OCR_TRIGGER_OBSERVABILITY_FAIL`, `reports/ocr_xvla_trace_observability_result.json`); OCR is archived. New claim-specific condition verified: wrist-camera dropout partial observation on `libero_spatial/task5` clean identities `20260731/32` dropped X-VLA from clean `2/2` to dropout `0/2`, `reports/wrist_dropout_condition_verification_result.json`; next select exactly one method, then executable Stage 0 within steer budget.

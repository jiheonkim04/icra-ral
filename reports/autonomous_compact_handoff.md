...
Repeated-residual screen `20260729..35`: natural-reset mining ended at `ddbe62c` with `NATURAL_RESET_SEARCH_SATURATED`; no more identity sweeps. SGL closed; OCR legal trace pass failed and is archived. AWF-XVLA Stage 0 no-go and reclassified as `SIMPLE_CAMERA_IMPUTATION_CONTROL`. Wrist-dropout generality confirmed with frozen X-VLA only: clean `9/9`, dropout `0/9`, flips `9`, decision `WRIST_DROPOUT_REPEATED_PROBLEM_CONFIRMED`. RL4IL selected as closest external prior; clone/compile/HDF5/CLIP smoke passed, but official release has constant-label supervision and no checkpoints, so comparator rollout is not armed as-is; next is checkpoint/upstream fix or mechanism-faithful action-sequence-oracle port before Ours. See `reports/rl4il_external_prior_selection_result.json` and `reports/rl4il_official_code_fidelity_audit_result.json`.

...
Repeated-residual screen `20260729..35`: natural-reset mining ended at `ddbe62c` with `NATURAL_RESET_SEARCH_SATURATED`; no more identity sweeps. Closed task5 family remains Base/X-VLA/OpenVLA failures on `20260727/30/33` with clean X-VLA retention `20260731/32`; SGL stays closed by simple-control equivalence. OCR got its only allowed five-episode legal trace pass and failed (`OCR_TRIGGER_OBSERVABILITY_FAIL`, `reports/ocr_xvla_trace_observability_result.json`); OCR is archived. Next axis selected/preregistered without Ours acronym: claim-specific wrist-camera dropout partial observation on `libero_spatial/task5` clean identities `20260731/32`, `reports/wrist_dropout_condition_preregistration_result.json`; next run exactly two frozen-prior dropout episodes and verify degradation before choosing one method.

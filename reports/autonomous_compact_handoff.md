...
Repeated-residual screen `20260729..35`: natural-reset mining ended at `ddbe62c` with `NATURAL_RESET_SEARCH_SATURATED`; no more identity sweeps. SGL closed; OCR legal trace pass failed and is archived. AWF-XVLA Stage 0 no-go and reclassified as `SIMPLE_CAMERA_IMPUTATION_CONTROL`. Wrist-dropout generality confirmed with frozen X-VLA only: 3 tasks x 3 identities clean/dropout paired, clean `9/9`, dropout `0/9`, flips `9`, McNemar p `0.00390625`, decision `WRIST_DROPOUT_REPEATED_PROBLEM_CONFIRMED`, `reports/wrist_dropout_generality_result.json`. RL4IL selected as closest external prior; official code cloned/compiled, local LIBERO HDF5 + CLIP feature smoke passed, but matched mask_1 prior rollout/residual-gap proof remains required before Ours; see `reports/rl4il_external_prior_selection_result.json`.

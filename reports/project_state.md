...
## Latest Route Result: ContactTube-Aug

Branch:

```text
codex/contacttube-aug-state0-state1
```

STATE 0 initialized the concise ContactTube-Aug docs. STATE 1 then ran a bounded local LIBERO/RoboSuite replay smoke using local HDF5 demos and the exact/default-reset replay infrastructure.

Result:

- replay/control metric produced: yes,
- total simulator steps: `1621`,
- variants: `6`,
- exact-init no-op upper bound succeeded: `true`,
- HDF5 object pose available: `false`,
- runtime replay object pose available: `true`,
...
- simple object-relative retarget tube score: `0.009154`,
- ContactTube-Aug beats simple object-relative retargeting: `false`,
- decision: kill before STATE 2.

Training, LoRA training, loss computation, downloads, GPU jobs, heavy model imports, OpenVLA-OFT execution, benchmark rollouts, and paper-grade claims did not occur. Do not continue to ContactTube-Aug STATE 2 unless a new method is predeclared and re-passes the simple-baseline replay gate.

Archive status: complete as documentation-only archive on branch `codex/contacttube-aug-archive`.
...
Global lesson added: a data-augmentation method is invalid for continuation if generated actions are not controller-valid, or if simple object-relative retargeting preserves trajectory/contact metrics better than the proposed augmentation.

Archive execution boundary: no new experiment, replay, rollout, training, loss computation, GPU job, download, OpenVLA-OFT execution, model loading, or paper claim occurred.
